"""Build the narrow wrapper-image manifest from explicit build identities."""
...
def build_manifest() -> dict[str, str]:
    ...
    if len(values["git_commit"]) != 40 or values["role"] not in {"original", "current"}:
        raise DiagnosticError("Top30 diagnostic image build identity differs")
    if not values["base_image_id"].startswith("sha256:"):
        raise DiagnosticError("Top30 diagnostic base image identity differs")
    return {"schema_version": "m6-top30-diagnostic-image-manifest-v1", **values}
