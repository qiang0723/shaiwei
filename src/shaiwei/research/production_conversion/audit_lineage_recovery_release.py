"""Build the metadata-only Head30 audit-lineage recovery scope."""
...
CONTRACT_PATH = MODULE_ROOT / "audit_lineage_recovery_contract.py"
ENTRYPOINT_PATH = MODULE_ROOT / "audit_lineage_recovery_entrypoint.py"
RELEASE_BUILDER_PATH = MODULE_ROOT / "audit_lineage_recovery_release.py"
...
DOCKERFILE_PATH = PROJECT_ROOT / "Dockerfile.m6-production-head30-audit-lineage-recovery"
R4_RELEASE_PATH = PROJECT_ROOT / "config/m6_csi800_production_head30_audit_entrypoint_recovery_scope_v1.json"
R4_APPROVAL_PATH = PROJECT_ROOT / "data/control/m6_csi800_production_head30_v1/audit-entrypoint-recovery-approval.json"
...
AUDIT_ROOT = PROJECT_ROOT / "data/research/m6_csi800_production_head30_v1/effect-r2-audit-lineage-entry-recovery"
...
def _origin_main() -> str:
    ...
    if len(value) != 40:
        raise ProtocolError("Head30 audit-lineage origin/main identity is invalid")
    return value


def verify_inputs(protocol: LineageProtocol) -> dict[str, Any]:
    r4 = protocol.document["r4_authority"]
    failure = protocol.document["r4_execution_failure"]
    exact = {
        R4_RELEASE_PATH: r4["release_document_sha256"],
        R4_APPROVAL_PATH: r4["approval_sha256"],
        R4_FAILURE_PATH: failure["evidence_sha256"],
        R3_PROTOCOL_PATH: protocol.document["root_cause_and_only_change"]["r3_protocol_sha256"],
    ...
    if any(sha256_file(path) != digest for path, digest in exact.items()):
        raise ProtocolError("Head30 audit-lineage predecessor file changed")
    ...
    if observed != {key: sealed[key] for key in ("file_count", "total_bytes", "tree_sha256")}:
        raise ProtocolError("Head30 sealed R2 effect changed before R5 release")
    artifacts = {
    ...
    if any(sha256_file(EFFECT_ROOT / path) != sealed[key] for key, path in artifacts.items()):
        raise ProtocolError("Head30 sealed R2 key artifact changed before R5 release")
    if AUDIT_ROOT.exists() and any(AUDIT_ROOT.iterdir()):
        raise ProtocolError("Head30 R5 audit output already exists")
    return expected_sealed(protocol)


def verify_daemon_preflight(
    protocol: LineageProtocol, *, image_id: str, image_git_commit: str
) -> dict[str, Any]:
    ...
    expected = {
        ...
        "r4_release_document_sha256": protocol.document["r4_authority"]["release_document_sha256"],
        "r4_release_scope_sha256": protocol.document["r4_authority"]["release_scope_sha256"],
        "r3_protocol_path": R3_PROTOCOL_TARGET,
        "r3_protocol_sha256": protocol.document["root_cause_and_only_change"]["r3_protocol_sha256"],
        "r3_release_document_sha256": "b6f385911832e104b04ca3354e3ec385af92645f24a2fc81a0c5f7fb6d9a40bd",
        "r3_release_scope_sha256": "b38628defcfee83087f0c0d982d0c1145b3f6d642c28508055cba2bddb9614d3",
        "r4_failure_evidence_sha256": protocol.document["r4_execution_failure"]["evidence_sha256"],
        "original_release_scope_sha256": "9b78ef69ec11c180bbc1adc46b95c3f8023bf729480d4fd647e2eab1085f9b4a",
        "original_approval_sha256": "0fe053c832897632d8cd8fbbb165252580b4134a44b62e27e9f416ebe4f47336",
        "original_protocol_sha256": protocol.document["root_cause_and_only_change"]["original_r2_protocol_sha256"],
    ...
    if evidence != expected:
        raise ProtocolError("Head30 audit-lineage daemon preflight evidence differs")
    return {
        "status": "PASS", "evidence_sha256": sha256_file(PREFLIGHT_EVIDENCE_PATH),
        "r3_protocol_path": evidence["r3_protocol_path"],
        "r3_protocol_sha256": evidence["r3_protocol_sha256"],
        "r3_release_scope_sha256": evidence["r3_release_scope_sha256"],
        "original_release_scope_sha256": evidence["original_release_scope_sha256"],
...
def build_release_document(
    *, protocol: LineageProtocol, created_at: str,
    implementation_git_commit: str, origin_main_commit: str,
    image_id: str, image_platform: str, image_git_commit: str,
    base_image_id: str, sealed_effect: dict[str, Any], daemon_preflight: dict[str, Any],
) -> dict[str, Any]:
    if implementation_git_commit != origin_main_commit or image_git_commit != implementation_git_commit:
        raise ProtocolError("Head30 audit-lineage implementation is not the pushed image commit")
    if base_image_id != protocol.document["r4_authority"]["image_id"]:
        raise ProtocolError("Head30 audit-lineage base image identity differs")
    implementation = {
    ...
    scope = {
        ...
        "r4_authority": protocol.document["r4_authority"],
        "r4_execution_failure": protocol.document["r4_execution_failure"],
        ...
        "image": {
            ...
            "base_reference": protocol.document["r4_authority"]["image_reference"],
        ...
        "daemon_preflight": daemon_preflight, "authority": expected_authority(),
    }
    document = {
        "schema_version": "m6-production-head30-audit-lineage-entry-recovery-scope-v1",
...
def build(
    *, image_id: str, image_platform: str, image_git_commit: str,
    ...
    if output.resolve() != SCOPE_PATH.resolve():
        raise ProtocolError("Head30 audit-lineage scope output path differs")
    protocol = LineageProtocol.load(PROTOCOL_PATH)
    document = build_release_document(
        ...
        daemon_preflight=verify_daemon_preflight(protocol, image_id=image_id, image_git_commit=image_git_commit),
    )
