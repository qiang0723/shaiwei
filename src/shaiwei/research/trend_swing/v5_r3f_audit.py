"""Independent zero-call audit of the TS-v5-R3F scope preflight."""
...
from shaiwei.research.trend_swing.v5_r3f_canary import OUTPUT_ROOT, REPORT_PATH, preflight

AUDIT_PATH = OUTPUT_ROOT / "preflight_audit.json"
...
def audit_report(report_path: Path = REPORT_PATH) -> dict[str, object]:
    ...
    try:
        ...
        raise D1ControlError("TS-v5-R3F preflight report is missing or invalid") from exc
    checks = {
    ...
    if not all(checks.values()):
        raise D1ControlError("TS-v5-R3F independent preflight audit failed")
    return {
        "schema_version": "ts-v5-r3f-canary-preflight-audit-v1",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(canonical_json({"status": "FAIL", "error_class": "TSV5R3FPreflightAuditError"}))
        return 2
