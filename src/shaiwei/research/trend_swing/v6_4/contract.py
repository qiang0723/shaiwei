"""Frozen contract and immutable paths for the TS-v6-4 no-take-profit effect."""
...
class V64Error(RuntimeError):
    """Fail-closed TS-v6-4 contract violation."""


PROTOCOL_PATH = PROJECT_ROOT / "config/ts_v6_4_no_takeprofit_effect_v1.yaml"
PROTOCOL_SHA256 = "bb73d92eec0bc4e5bb6ea6b671e4f40cc3dfdf6f1c28304890397597c2201502"
OUTPUT_ROOT = PROJECT_ROOT / "data/research/trend_swing/ts-v6-4-no-takeprofit-effect-v1"
PARENT_POINT_HASH = "81833a47b1edb59455c997c422bb36b63454f1da84e29696269c9c950e019784"
V63_FIRST_PASS_ROOT = PROJECT_ROOT / (
    "data/research/trend_swing/ts-v6-3-ranked-subset-effect-v1-r2/first_pass"
)
V63_FIRST_PASS_BUNDLE_SHA256 = (
    "056bf4d506bc4d76910c5e61d3c3ba76cf86475a1c55f317f417ce659e5cfcba"
)


def _mapping(path: Path) -> dict[str, Any]:
    if path.is_symlink() or sha256_file(path) != PROTOCOL_SHA256:
        raise V64Error(f"TS-v6-4 frozen protocol differs: {path.name}")
    try:
        value = yaml.safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as exc:
        raise V64Error(f"TS-v6-4 frozen YAML is invalid: {path.name}") from exc
    if not isinstance(value, dict):
        raise V64Error(f"TS-v6-4 frozen YAML is not a mapping: {path.name}")
    return value
...
class V64Scope:
    ...
    def load(cls) -> "V64Scope":
        ...
        change = document.get("exit_mechanism_change", {})
        if (
            document.get("schema_version") != "ts-v6-4-no-takeprofit-effect-protocol-v1"
            ...
            or point.get("event_subset") != "full_parent_188_discovery_events_no_quality_filter"
            or change.get("fixed_take_profit") != "REMOVED"
            or change.get("no_new_exit_or_threshold_added") is not True
            or change.get("stop_risk_time_parameters_unchanged") is not True
            ...
            raise V64Error("TS-v6-4 authority, role, or effect contract differs")
        return cls(document)
...
def validate_authorized_effect_inputs(scope: V64Scope, root: Path = PROJECT_ROOT) -> None:
    checks: dict[str, str] = {}
    ...
    lineage = scope.document["ranking_lineage"]["clean_m6_lineage"]
    for row in lineage["reusable_predictions"].values():
        checks[row["path"]] = row["sha256"]
    for relative, expected in checks.items():
        path = root / relative
        if path.is_symlink() or not path.is_file() or sha256_file(path) != expected:
            raise V64Error(f"TS-v6-4 bound input differs: {relative}")
...
def runtime_identity() -> dict[str, str]:
    ...
    if re.fullmatch(r"[0-9a-f]{40}", embedded) is None or git_head() != embedded:
        raise V64Error("TS-v6-4 release Git identity differs")
