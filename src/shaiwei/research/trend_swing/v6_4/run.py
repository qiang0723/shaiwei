"""One-shot TS-v6-4 discovery effect runner with write-once evidence."""
...
from shaiwei.research.trend_swing.v6_4.inputs import V64Adapter
from shaiwei.research.trend_swing.v6_4.metrics import legacy_sharpes
...
def execute_discovery(
    ...
    if events.empty:
        raise V64Error("TS-v6-4 candidate events are empty")
    ...
    gate = evaluate_candidate(summaries, base_nav, base_trades, gate_config, legacy)
    document = {
        "schema_version": "ts-v6-4-discovery-effect-v1",
        "partition": "discovery",
        "point": point,
        ...
        "gate": gate,
        "verdict": (
            "GO_TS_V6_4_DRAFT_HOLDOUT_PROTOCOL_REQUIRES_NEW_BUDGET_DECISION"
            if gate["passed"]
            else "REJECT_TS_V6_4_NO_TAKEPROFIT_DISCOVERY_AND_LANE_CLOSES"
        ),
...
def _execute_pass(
    root: Path, scope: V64Scope, adapter: V64Adapter, legacy: Sequence[float]
) -> dict[str, Any]:
    discovery = execute_discovery(root, scope, adapter, legacy)
    summary = {
        "schema_version": "ts-v6-4-effect-pass-summary-v1",
...
def run_once() -> dict[str, Any]:
    ...
    if marker_path.exists() or report_path.exists() or failure_path.exists():
        raise V64Error("TS-v6-4 effect output exists; same-scope rerun is forbidden")
    if not preflight_path.is_file():
        raise V64Error("TS-v6-4 key-only preflight must be sealed before the effect read")
    try:
        preflight = json.loads(preflight_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise V64Error("TS-v6-4 sealed preflight is invalid") from exc
    effect_started = False
    try:
        adapter = V64Adapter(scope, OUTPUT_ROOT / "duckdb-tmp")
        recomputed = adapter.preflight()
        if canonical_json(recomputed) != canonical_json(preflight):
            raise V64Error("TS-v6-4 pre-effect key preflight identity differs")
        legacy = legacy_sharpes()
        write_once_json(
            marker_path,
            {
                "schema_version": "ts-v6-4-effect-read-marker-v1",
        ...
        if any(first[key] != replay[key] for key in comparable):
            raise V64Error("TS-v6-4 first pass and replay differ")
        report = {
            "schema_version": "ts-v6-4-effect-report-v1",
            "protocol_sha256": scope.sha256,
            "release_identity": identity,
        ...
        write_once_json(
            failure_path,
            {
                "schema_version": "ts-v6-4-effect-failure-v1",
                "protocol_sha256": scope.sha256,
...
def preflight_once() -> dict[str, Any]:
    ...
    if any(
        ...
        raise V64Error("TS-v6-4 preflight or effect output exists; rerun is forbidden")
    adapter = V64Adapter(scope, OUTPUT_ROOT / "duckdb-tmp-preflight")
