"""Independent artifact auditor for the one-shot TS-v6-4 discovery effect."""
...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise V64Error(f"TS-v6-4 audit input is invalid: {path.name}") from exc
    if not isinstance(value, dict):
        raise V64Error(f"TS-v6-4 audit input is not an object: {path.name}")
    return value
...
def _manifest(root: Path) -> dict[str, Any]:
    ...
    if document != tree_manifest(root):
        raise V64Error("TS-v6-4 independent artifact manifest differs")
    return document
...
def audit_once() -> dict[str, Any]:
    ...
    if audit_path.exists():
        raise V64Error("TS-v6-4 audit exists; same-scope audit rerun is forbidden")
    scope = V64Scope.load()
    ...
    if not _pass_trees_equal(first, replay):
        raise V64Error("TS-v6-4 first pass and replay artifact trees differ")
    ...
    for name in SCENARIOS:
        ...
        if json.loads(canonical_json(written)) != json.loads(canonical_json(recomputed)):
            raise V64Error(f"TS-v6-4 audited summary differs: {name}")
    ...
    adapter = V64Adapter(scope, OUTPUT_ROOT / "duckdb-tmp-audit")
    ...
    checks = {
        ...
        "no_take_profit_mechanism_removed": "TAKE_PROFIT" not in reasons,
    ...
    audit = {
        "schema_version": "ts-v6-4-no-takeprofit-effect-independent-audit-v1",
    ...
    if verdict != "PASS":
        raise V64Error("TS-v6-4 independent audit failed")
    return audit
