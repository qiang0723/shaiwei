"""Independent offline audit for a completed TS-v5-R3C six-response canary."""
...
from shaiwei.research.trend_swing.v5_r3c_live import (
...
def _linked_response(
    *, ordinal: int, row: dict[str, str], request: dict[str, Any],
    by_attempt: dict[str, dict[str, str]], output_root: Path, project_root: Path,
) -> dict[str, Any]:
    ...
    if (
        ...
        raise D1ControlError("TS-v5-R3C attempt schedule differs")
    ...
    if request_text != canonical_json(request) or re.search(
        ...
        raise D1ControlError("TS-v5-R3C outbound request differs or contains identity")
    ...
    if sha256_file(provider_path) != event["response_artifact_sha256"]:
        raise D1ControlError("TS-v5-R3C provider artifact differs")
    return raw


def _verify_classification(
    *, ordinal: int, row: dict[str, str], raw: dict[str, Any],
    prior_signatures: set[str],
) -> None:
    classified = classify_proposal_response(
        MECHANISMS[ordinal - 1], provider_response(raw, label="TS-v5-R3C"),
    ...
    if observed != recorded or abs(classified["cost"] - float(row["estimated_cost_usd"])) > 5e-13:
        raise D1ControlError("TS-v5-R3C independent classification differs")
...
def _verify_attempts(
    *, rows: list[dict[str, str]], transport: list[dict[str, str]],
    release: R3CExecutionRelease, output_root: Path, project_root: Path,
) -> dict[str, Any]:
    completed = [row for row in transport if row["event_type"] == "COMPLETED"]
    by_attempt = {row["attempt_id"]: row for row in completed}
    if len(completed) != 6 or len(by_attempt) != 6:
        raise D1ControlError("TS-v5-R3C audit requires exactly six completed transports")
    ...
    for ordinal, (row, request) in enumerate(zip(rows, requests, strict=True), start=1):
        raw = _linked_response(
            ordinal=ordinal, row=row, request=request, by_attempt=by_attempt,
            output_root=output_root, project_root=project_root,
        )
        _verify_classification(
            ordinal=ordinal, row=row, raw=raw, prior_signatures=prior_signatures
        )
    return {
...
def audit_batch(
    *, release_path: Path, output_root: Path, attempt_path: Path, transport_path: Path,
    ...
    release = R3CExecutionRelease.load(release_path, protocol)
    rows = attempt_rows(attempt_path, release)
    if len(rows) != 6:
        raise D1ControlError("TS-v5-R3C audit requires exactly six attempt rows")
    static = _verify_attempts(
        rows=rows, transport=transport_rows(transport_path, release, label="TS-v5-R3C"),
        release=release, output_root=output_root, project_root=project_root,
    )
    report = json_object(output_root / "ts_v5_r3c_report.json", label="TS-v5-R3C")
    valid = sum(
    ...
    if not all(checks.values()):
        raise D1ControlError("TS-v5-R3C independent audit failed")
    return {
        "schema_version": "ts-v5-r3c-independent-audit-v1",
        "release_sha256": release.sha256, "checks": checks, "static_evidence": static,
        "actual_cost_usd": cost, "valid_unique_candidate_count": valid,
        "authoritative_gate": gate, "network_used": False, "secret_read": False,
        "candidate_effectiveness": "NOT_EVALUATED", "production_authorization": "none",
        "verdict": "PASS",
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(canonical_json({"status": "FAIL", "error_class": "TSV5R3CResultAuditError"}))
        return 2
