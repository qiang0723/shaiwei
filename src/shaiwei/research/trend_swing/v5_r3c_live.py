"""One-shot six-response runner for a separately approved TS-v5-R3C release."""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "data/research/trend_swing/ts-v5-r3c-canary-001"
DEFAULT_ATTEMPT_LEDGER = PROJECT_ROOT / "ledger/ts_v5_r3c_llm_attempts.csv"
DEFAULT_TRANSPORT_LEDGER = PROJECT_ROOT / "ledger/ts_v5_r3c_llm_transports.csv"
WORST_CASE_ATTEMPT_USD = (16_000 * 0.435 + 1_800 * 0.87) / 1_000_000


def _load_release(path: Path) -> tuple[V5TransportProtocol, R3CExecutionRelease]:
...
def run_preflight(
    *,
    ...
    _, release = _load_release(release_path)
    if (
        runtime_git_head() != release.implementation_git_head
        or runtime_code_sha() != release.code_snapshot_sha256
    ):
        raise D1ControlError("TS-v5-R3C runtime identity differs")
    if attempt_rows(attempt_path, release) or transport_path.read_text(encoding="utf-8").splitlines()[1:]:
        raise D1ControlError("TS-v5-R3C preflight requires pristine dedicated ledgers")
    bundle = sha256_text(canonical_json(request_bundle()))
    if bundle != REQUEST_BUNDLE_SHA256:
        raise D1ControlError("TS-v5-R3C prepared request bundle differs")
    return {
        "schema_version": "ts-v5-r3c-live-preflight-v1",
...
def _terminal_report(
    release: R3CExecutionRelease, rows: list[dict[str, str]], *, tls_sha: str,
) -> dict[str, Any]:
    valid = sum(
        not row["failure_class"] and row["schema_status"] == "PASS"
        and row["duplicate_status"] == "UNIQUE" for row in rows
    )
    return {
        "schema_version": "ts-v5-r3c-canary-report-v1",
        "release_sha256": release.sha256,
        ...
        "production_authorization": "none", "gate": batch_gate(len(rows), valid),
    }


def _run_attempt(
    *, ordinal: int, request: dict[str, Any], protocol: V5TransportProtocol,
    release: R3CExecutionRelease, output_root: Path, attempt_path: Path,
    transport_path: Path, prior_signatures: set[str], provider_factory: Callable[..., Any],
    project_root: Path, code_sha: str,
) -> int:
    ...
    with provider_factory(
        protocol, release=release, attempt_id=task["attempt_id"],
        transport_ledger_path=transport_path, artifact_root=output_root / "artifacts/provider",
    ) as provider:
    ...
    classified = classify_proposal_response(
        MECHANISMS[ordinal - 1], response, prior_semantic_signatures=prior_signatures
    )
    ...
    persist_completed_attempt(
        ...
        raw_path=raw_path, attempt_path=attempt_path, output_root=output_root,
        code_sha=code_sha, project_root=project_root, operator="docker-ts-v5-r3c",
    )
...
def run_batch(
    *,
    ...
    provider_factory: Callable[..., Any] = create_r3c_provider,
    tls_probe: Callable[[R3CExecutionRelease], str] = tls_hostname_probe,
    runtime_git_head: Callable[[], str] = git_head,
    runtime_code_sha: Callable[[], str] = code_snapshot_sha256,
) -> dict[str, Any]:
    ...
    if report_path.is_file():
        ...
        if report.get("release_sha256") != release.sha256 or len(rows) != 6:
            raise D1ControlError("TS-v5-R3C terminal report identity differs")
    ...
    for ordinal in range(len(rows) + 1, 7):
        rows = attempt_rows(attempt_path, release)
        spent = sum(float(row["estimated_cost_usd"]) for row in rows)
        if spent + WORST_CASE_ATTEMPT_USD > release.batch_hard_ceiling_usd:
            raise D1ControlError("TS-v5-R3C cost reserve reaches the hard ceiling")
        calls += _run_attempt(
            ordinal=ordinal, request=requests[ordinal - 1], protocol=protocol,
            release=release, output_root=output_root, attempt_path=attempt_path,
            transport_path=transport_path, prior_signatures=prior_signatures,
            provider_factory=provider_factory, project_root=project_root, code_sha=code_sha,
        )
    rows = attempt_rows(attempt_path, release)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        kwargs = {"release_path": args.release, "attempt_path": args.attempt_ledger,
                  "transport_path": args.transport_ledger}
        result = run_preflight(**kwargs) if args.preflight_only else run_batch(
            **kwargs, output_root=args.output_root
        )
    except (D1ControlError, OSError, RuntimeError, TypeError, ValueError, json.JSONDecodeError):
        print(canonical_json({"status": "FAIL", "error_class": "TSV5R3CExecutionError"}))
        return 2
