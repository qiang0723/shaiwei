"""Build the exact metadata-only M6-5B-R1 release scope."""
...
from shaiwei.provenance import (
    RELEASE_MANIFEST_SCHEMA, code_snapshot_sha256, git_head, verify_release_manifest,
)
...
from .entrypoint_recovery_contract import (
    ACTION, COMPOSE_PATH, IMAGE, SCOPE_KIND, SCOPE_SCHEMA,
    EntrypointRecoveryProtocol, expected_authority, validate_scope,
)
...
def _origin_main() -> str:
    ...
    if len(value) != 40:
        raise ProtocolError("M6-5B-R1 origin/main identity is invalid")
    return value


def build_document(
    *, protocol: EntrypointRecoveryProtocol, created_at: str, commit: str, origin: str,
    ...
    if (
        image_manifest.get("schema_version") != RELEASE_MANIFEST_SCHEMA
        or image_manifest.get("code_snapshot_sha256") != snapshot
    ):
        raise ProtocolError("M6-5B-R1 image manifest differs")
    raw = mapping(raw_manifest_path)
    r2 = effect_tree_identity(r2_root)
    expected_r2 = protocol.base.document["predecessors"]["sealed_r2_effect"]
    expected_identity = {key: expected_r2[key] for key in ("file_count", "total_bytes", "tree_sha256")}
    if r2 != expected_identity:
        raise ProtocolError("M6-5B-R1 sealed R2 identity differs")
    if sha256_file(r7_audit) != expected_r2["independent_audit_sha256"]:
        raise ProtocolError("M6-5B-R1 R7 audit identity differs")
    release = protocol.document["release_and_approval"]
    scope = {
        "scope_kind": SCOPE_KIND, "created_at": created_at,
        "entrypoint_recovery_protocol_sha256": protocol.sha256,
        "base_protocol_sha256": protocol.base.sha256,
        "target_read_recovery_sha256": protocol.base.recovery_sha256,
        "implementation": {
            "git_commit": commit, "origin_main_commit": origin,
        ...
        "inputs": {
            ...
            "r7_audit": {
                "path": str(r7_audit.relative_to(PROJECT_ROOT)), "sha256": sha256_file(r7_audit),
            },
            "raw_batch_manifest": {
        ...
        "container": {
            "compose_path": COMPOSE_PATH.name, "compose_sha256": sha256_file(COMPOSE_PATH),
        ...
        "outputs": {
            "effect_root": release["effect_root"], "audit_root": release["audit_root"],
            "approval_path": release["approval_record_path"],
            "raw_manifest_path": "data/control/m6_csi800_production_head30_500k_feasibility_v1/raw-batch-manifest.json",
            "create_host_path": False, "write_once": True,
    ...
    return {
        ...
        "release_scope_sha256": canonical_sha256(scope), "scope": scope,
    }
...
def build(
    *, image_id: str, image_platform: str, image_manifest: Path, r2_root: Path,
    ...
    paths = [image_manifest, r2_root, r7_audit, raw_manifest, output]
    image_manifest, r2_root, r7_audit, raw_manifest, output = [path.resolve() for path in paths]
    protocol = EntrypointRecoveryProtocol.load()
    head, origin = git_head(), _origin_main()
    if head != origin:
        raise ProtocolError("M6-5B-R1 implementation is not synchronized")
    snapshot = code_snapshot_sha256()
    if verify_release_manifest(image_manifest, root=PROJECT_ROOT) != snapshot:
        raise ProtocolError("M6-5B-R1 embedded manifest does not match host")
    ...
    return {
        ...
        "execution_authorized": False, "family_attempts_before_run": 1,
