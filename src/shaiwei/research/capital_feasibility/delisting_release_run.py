"""Claim-first one-shot M6-5C delisting-risk historical diagnostic runner."""
...
def _empty(root: Path) -> None:
    ...
    if any(root.iterdir()):
        raise ProtocolError("M6-5C effect root is not empty")
...
def _read_and_run(
    receipt: dict[str, object], *, release: ReleaseScope, r2_root: Path,
    r7_audit: Path, raw_manifest: Path, project_root: Path,
) -> dict[str, Any]:
    verify_tree(r2_root, release)
    expected_r7 = release.scope["inputs"]["r7_audit"]
    if sha256_file(r7_audit) != expected_r7["sha256"]:
        raise ProtocolError("M6-5C R7 audit identity differs")
    raw = release.scope["inputs"]["raw_batch_manifest"]
    if sha256_file(raw_manifest) != raw["sha256"]:
        raise ProtocolError("M6-5C raw manifest identity differs")
    ...
    if first != replay:
        raise ProtocolError("M6-5C internal replay differs")
    return {"receipt": receipt, "first": first, "replay": replay}


def execute_loaded(
    *, release: ReleaseScope, approval: Approval, r2_root: Path, r7_audit: Path,
    raw_manifest: Path, project_root: Path, ledger_path: Path, receipt_path: Path,
    output_root: Path, effect_reader: Callable[[dict[str, object]], dict[str, Any]] | None = None,
    ...
    reader = effect_reader or (
        lambda receipt: _read_and_run(
            receipt, release=release, r2_root=r2_root, r7_audit=r7_audit,
            raw_manifest=raw_manifest, project_root=project_root,
        )
    )
    try:
        claim_invoked = True
        payload = read_effect_after_claim(
            _spec(release), ledger_path=ledger_path, receipt_path=receipt_path,
            effect_reader=reader, claimed_at=claimed_at,
        )
        receipt = payload["receipt"]
        first, replay = payload["first"], payload["replay"]
        authorization = {
            "schema_version": "m6-head30-500k-delisting-risk-authorization-v1",
            "release_scope_sha256": release.sha256,
        ...
        if first_sha != replay_sha:
            raise ProtocolError("M6-5C physical replay differs")
        result = first["result"]
        report = {
            "schema_version": "m6-head30-500k-delisting-risk-effect-report-v1",
            "release_scope_sha256": release.sha256,
            "approval_sha256": approval.sha256,
            "claim_receipt_sha256": receipt["receipt_sha256"],
            "experiment_id": receipt["experiment_id"],
        ...
        write_once_document(output_root / "failure.json", {
            "schema_version": "m6-head30-500k-delisting-risk-effect-failure-v1",
            "release_scope_sha256": release.sha256,
            "approval_sha256": approval.sha256,
...
def main(argv: list[str] | None = None, *, executor: Any = run) -> int:
    ...
    for name in (
        "release", "approval", "r2-root", "r7-audit", "raw-manifest",
        "project-root", "ledger", "claim-receipt", "output-root",
    ):
    ...
    result = executor(
        ...
        r2_root=values["r2_root"], r7_audit=values["r7_audit"],
        raw_manifest=values["raw_manifest"], project_root=values["project_root"],
        ledger_path=values["ledger"], receipt_path=values["claim_receipt"],
