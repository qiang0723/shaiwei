"""Frozen SQLite schema descriptor and fingerprint for the M5-2 registry."""
...
EXPECTED_SCHEMA_FINGERPRINT = "9fcc4a3763041cf676219245f49bffacc07a595e449cb9066c7a508e6feb562a"


class SchemaFingerprintError(RuntimeError):
    """The operational schema differs from M5-2 registry v1."""
...
def schema_descriptor(connection: sqlite3.Connection) -> dict[str, Any]:
    ...
    object_rows = [
        {"type": row[0], "name": row[1], "table": row[2], "sql": _normalized_sql(row[3])}
        for row in objects
    ]
...
def validate_schema_fingerprint(connection: sqlite3.Connection) -> None:
    if schema_fingerprint(connection) != EXPECTED_SCHEMA_FINGERPRINT:
        raise SchemaFingerprintError("M5-2 registry schema fingerprint mismatch")
