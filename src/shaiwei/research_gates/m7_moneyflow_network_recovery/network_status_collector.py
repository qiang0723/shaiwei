"""Approved-scope Baostock status collector for one sealed M7 request plan."""
...
from .live_clients import baostock_session
from .network_collect import collect_status_plan
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        claim_role_once(
            ...
            run_id=role_activation_id(authority, "status_collector"),
        )
        plan = load_runtime_plan(authority)
        with baostock_session() as client:
            result = collect_status_plan(
                authority.recovery,
                plan,
                release_scope_sha256=authority.release.sha256,
                client=client,
