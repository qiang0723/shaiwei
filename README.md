...
- 施工入口：AGENTS.md ｜ 当前状态：STATE.md ｜ 结果路线图：docs/ROADMAP.md ｜ 口径卡：docs/DATA_SPEC.md ｜ 哨兵：docs/SENTINELS.md ｜ 判据：docs/GATES.md ｜ 日程：docs/DAY_PLAN.md ｜ 产品篮子：docs/FUND_COMPARATOR_SPEC.md
...
交易日历与 `ledger/daily_runs.csv` 对账；北京时间 16:00 起对当日五类输入做内存探测，数据完整即
正式跑批，未齐不写raw/账本也不发失败告警；19:30 后恢复硬失败与告警兜底。电脑熄屏不影响，
...
`g8-v1` 已冻结逐产品成对、滞后 60 日波动率、只降风险不加杠杆的比较公式，以及 4/6 产品和
2/3 年度子期稳定性门槛。三年数据形成前只能查看规格，不会提前输出结论：

```bash
make g8-spec
```

完整输入与审计契约见 `docs/G8_SPEC.md`。
...
`g1-v1` 把因子准入规则实现为 fail-closed 的统一裁判。它从追加式实验总账机械统计同研究家族的
全部尝试 N（失败也计数），以 `DSR≥0.95` 和 `Newey-West(10) t≥3.0` 两个独立统计门，加上
PIT/shift、复杂度、对库相关性、六窗口衰减、三压力期、换手、净 ICIR/净超额与双倍成本/滑点，
共同输出 `PASS/REJECT`。判定绑定证据、规则、实验账本、代码和数据哈希，写入独立追加账本，复跑
不会重复决策。完整冻结口径和输入契约见 `docs/G1_SPEC.md`；`make g1-schema` 打印机器 schema，
`make g1-admit G1_EVIDENCE=...` 或 Docker 同名入口只运行裁判，不生成因子。

项目另提供与裁判分离的有界预演流水线：中证800、固定 seed、40 个 GP 候选、1 代；请求发现期为
2016-01-01~2018-12-31。由于本地 qlib 数据从 2016 年首个交易日开始，AlphaGen 需要 100 个交易日
回溯，机器报告会同时记录请求起点和实际可评估起点（当前为 2016-06-01），禁止负索引绕到数据尾部。
有效候选至少要有 252 个日频 IC，按发现期 `|RankIC|` 排序，方向只冻结一次。

前两名依次经过安全 AST、嵌套累计回溯、合成未来扰动 shift 测试、行业/市值 PIT 中性化、W1-W6、
三压力期、Alpha158 90% + 因子 10% 同预算组合、成本翻倍及买卖各加 10bp。每次生成和组合评估都
写实验总账；汇总文件按 benchmark SHA-256 版本化。预演只输出 `PASS/REJECT`，即使 `PASS` 也不
自动写正式因子库，且占位经济解释不能代替人工陈述。

```bash
make docker-stage1-preflight  # Docker 内 GP → 自动证据 → 冻结 G1；正式入库始终为 0
make stage1-gp-preflight      # 仅生成并全量记账
make stage1-g1-preflight      # 对该家族最新、同代码/数据快照报告自动取证和裁决
```

机器证据位于 `data/research/g1/<family>/`，判定报告位于 `logs/g1/`；二者不进 Git，但路径与哈希会
进入追加式账本。完整执行口径和 2026-07-16 预演结果见 `docs/G1_SPEC.md` 与 `STATE.md`。
