...
const StrategyFactoryPage = lazy(() => import("./pages/StrategyFactoryPage"));
...
function NotFound() {
  return (
    <section className="request-error" role="alert">
      ...
      <h1>此只读页面未开放</h1>
      <p>当前只提供总览、策略工厂、因子工厂、模型/回测、模拟组合、股票池/信号、数据质量和系统运行。</p>
    </section>
  );
}
...
function AppRoutes() {
  ...
  if (location.pathname === "/") page = <RootRedirect />;
  ...
  else if (location.pathname === "/strategy-factory") page = <StrategyFactoryPage />;
  else if (
    ...
    /^\/experiments\/(?:research_experiment|p2_engineering_run|p2_effect_original|p2_effect_correction)\/[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(location.pathname)
