...
.PHONY: docker-m6-head30-500k-entrypoint-recovery-build docker-m6-head30-500k-entrypoint-recovery-fixture docker-m6-head30-500k-entrypoint-recovery-run docker-m6-head30-500k-entrypoint-recovery-audit
...
docker-ts-b-build: ## 从已推送提交断网构建TS-B留出期效果镜像
	@test -n "$(TS_B_RELEASE_GIT_HEAD)" || (echo "TS_B_RELEASE_GIT_HEAD is required"; exit 2)
	@test "$(TS_B_RELEASE_GIT_HEAD)" = "$$(git rev-parse HEAD)" || (echo "TS-B release Git differs from HEAD"; exit 2)
	@test "$(TS_B_RELEASE_GIT_HEAD)" = "$$(git rev-parse origin/main)" || (echo "TS-B release Git differs from origin/main"; exit 2)
	SHAIWEI_TS_B_RELEASE_GIT_HEAD="$(TS_B_RELEASE_GIT_HEAD)" docker compose -f compose.ts-b-holdout.yaml --profile ts-b-fixture build ts-b-effect-fixture

docker-ts-b-fixture: ## 真实读取前断网运行合成fixture与合同测试
	docker compose -f compose.ts-b-holdout.yaml --profile ts-b-fixture run --rm --no-deps ts-b-effect-fixture

docker-ts-b-preflight: ## 只读事件键与分数覆盖（不读收益）
	mkdir -p data/research/trend_swing/ts-b-holdout-effect-v1
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-b-holdout.yaml --profile ts-b-live run --rm --no-deps ts-b-effect-preflight

docker-ts-b-run: ## 唯一一次留出期效果读取（含确定性复算）
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-b-holdout.yaml --profile ts-b-live run --rm --no-deps ts-b-effect-runner

docker-ts-b-audit: ## 独立复核TS-B汇总、门禁与裁决
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-b-holdout.yaml --profile ts-b-live run --rm --no-deps ts-b-effect-auditor

docker-rf-0b-build: ## 从已推送提交断网构建RF-0B预检镜像
	@test -n "$(RF_0B_RELEASE_GIT_HEAD)" || (echo "RF_0B_RELEASE_GIT_HEAD is required"; exit 2)
	@test "$(RF_0B_RELEASE_GIT_HEAD)" = "$$(git rev-parse HEAD)" || (echo "RF-0B release Git differs from HEAD"; exit 2)
	@test "$(RF_0B_RELEASE_GIT_HEAD)" = "$$(git rev-parse origin/main)" || (echo "RF-0B release Git differs from origin/main"; exit 2)
	SHAIWEI_RF_0B_RELEASE_GIT_HEAD="$(RF_0B_RELEASE_GIT_HEAD)" docker compose -f compose.ts-rf-0b.yaml --profile rf-0b-fixture build rf-0b-fixture

docker-rf-0b-fixture: ## 真实画像前断网运行对抗fixture与合同测试
	docker compose -f compose.ts-rf-0b.yaml --profile rf-0b-fixture run --rm --no-deps rf-0b-fixture

docker-rf-0b-profile: ## 唯一一次字段画像与身份注册（不读收益）
	mkdir -p data/research/rf/rf-0b-field-identity-preflight-v1
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-rf-0b.yaml --profile rf-0b-live run --rm --no-deps rf-0b-profile

docker-rf-0b-audit: ## 独立复核RF-0B注册表、画像与门禁
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-rf-0b.yaml --profile rf-0b-live run --rm --no-deps rf-0b-auditor

docker-rf-diag-build: ## 从已推送提交断网构建RF缺口谱系诊断镜像
	@test -n "$(RF_DIAG_RELEASE_GIT_HEAD)" || (echo "RF_DIAG_RELEASE_GIT_HEAD is required"; exit 2)
	@test "$(RF_DIAG_RELEASE_GIT_HEAD)" = "$$(git rev-parse HEAD)" || (echo "RF diag release Git differs from HEAD"; exit 2)
	@test "$(RF_DIAG_RELEASE_GIT_HEAD)" = "$$(git rev-parse origin/main)" || (echo "RF diag release Git differs from origin/main"; exit 2)
	SHAIWEI_RF_DIAG_RELEASE_GIT_HEAD="$(RF_DIAG_RELEASE_GIT_HEAD)" docker compose -f compose.ts-rf-diag.yaml --profile rf-diag-fixture build rf-diag-fixture

docker-rf-diag-fixture: ## 真实诊断前断网运行合成fixture与合同测试
	docker compose -f compose.ts-rf-diag.yaml --profile rf-diag-fixture run --rm --no-deps rf-diag-fixture

docker-rf-diag-run: ## 唯一一次缺口谱系诊断（只读证据层，不读收益）
	mkdir -p data/research/rf/rf-0b-gap-lineage-diagnostic-v1
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-rf-diag.yaml --profile rf-diag-live run --rm --no-deps rf-diag-runner

docker-rf-diag-audit: ## 独立复核诊断键与解释类别
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-rf-diag.yaml --profile rf-diag-live run --rm --no-deps rf-diag-auditor

docker-rf-0c-build: ## 从已推送提交断网构建RF-0C预检镜像
	@test -n "$(RF_0C_RELEASE_GIT_HEAD)" || (echo "RF_0C_RELEASE_GIT_HEAD is required"; exit 2)
	@test "$(RF_0C_RELEASE_GIT_HEAD)" = "$$(git rev-parse HEAD)" || (echo "RF-0C release Git differs from HEAD"; exit 2)
	@test "$(RF_0C_RELEASE_GIT_HEAD)" = "$$(git rev-parse origin/main)" || (echo "RF-0C release Git differs from origin/main"; exit 2)
	SHAIWEI_RF_0C_RELEASE_GIT_HEAD="$(RF_0C_RELEASE_GIT_HEAD)" docker compose -f compose.ts-rf-0c.yaml --profile rf-0c-fixture build rf-0c-fixture

docker-rf-0c-fixture: ## 真实画像前断网运行对抗fixture与合同测试
	docker compose -f compose.ts-rf-0c.yaml --profile rf-0c-fixture run --rm --no-deps rf-0c-fixture

docker-rf-0c-profile: ## 唯一一次字段画像与身份注册复核（不读收益）
	mkdir -p data/research/rf/rf-0c-field-identity-preflight-v1
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-rf-0c.yaml --profile rf-0c-live run --rm --no-deps rf-0c-profile

docker-rf-0c-audit: ## 独立复核RF-0C注册表、画像与门禁
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-rf-0c.yaml --profile rf-0c-live run --rm --no-deps rf-0c-auditor

docker-rf-1-build: ## 从已推送提交断网构建RF-1合同镜像
	@test -n "$(RF_1_RELEASE_GIT_HEAD)" || (echo "RF_1_RELEASE_GIT_HEAD is required"; exit 2)
	@test "$(RF_1_RELEASE_GIT_HEAD)" = "$$(git rev-parse HEAD)" || (echo "RF-1 release Git differs from HEAD"; exit 2)
	@test "$(RF_1_RELEASE_GIT_HEAD)" = "$$(git rev-parse origin/main)" || (echo "RF-1 release Git differs from origin/main"; exit 2)
	SHAIWEI_RF_1_RELEASE_GIT_HEAD="$(RF_1_RELEASE_GIT_HEAD)" docker compose -f compose.ts-rf-1.yaml --profile rf-1-fixture build rf-1-fixture

docker-rf-1-fixture: ## 断网运行候选合同、去重与release门合成测试
	docker compose -f compose.ts-rf-1.yaml --profile rf-1-fixture run --rm --no-deps rf-1-fixture

docker-ts-c-build: ## 从已推送提交断网构建TS-C资格赛镜像
	@test -n "$(TS_C_RELEASE_GIT_HEAD)" || (echo "TS_C_RELEASE_GIT_HEAD is required"; exit 2)
	@test "$(TS_C_RELEASE_GIT_HEAD)" = "$$(git rev-parse HEAD)" || (echo "TS-C release Git differs from HEAD"; exit 2)
	@test "$(TS_C_RELEASE_GIT_HEAD)" = "$$(git rev-parse origin/main)" || (echo "TS-C release Git differs from origin/main"; exit 2)
	SHAIWEI_TS_C_RELEASE_GIT_HEAD="$(TS_C_RELEASE_GIT_HEAD)" docker compose -f compose.ts-c-qualification.yaml --profile ts-c-fixture build ts-c-fixture

docker-ts-c-fixture: ## 真实画像前断网运行合成fixture与合同测试
	docker compose -f compose.ts-c-qualification.yaml --profile ts-c-fixture run --rm --no-deps ts-c-fixture

docker-ts-c-profile: ## 唯一一次三臂触发器密度画像（不读收益）
	mkdir -p data/research/trend_swing/ts-c-trigger-qualification-v1
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-c-qualification.yaml --profile ts-c-live run --rm --no-deps ts-c-profile

docker-ts-c-audit: ## 独立复核TS-C密度与门禁
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-c-qualification.yaml --profile ts-c-live run --rm --no-deps ts-c-auditor

docker-ts-c-v2-build: ## 从已推送提交断网构建TS-C-v2资格赛镜像
	@test -n "$(TS_C_V2_RELEASE_GIT_HEAD)" || (echo "TS_C_V2_RELEASE_GIT_HEAD is required"; exit 2)
	@test "$(TS_C_V2_RELEASE_GIT_HEAD)" = "$$(git rev-parse HEAD)" || (echo "TS-C-v2 release Git differs from HEAD"; exit 2)
	@test "$(TS_C_V2_RELEASE_GIT_HEAD)" = "$$(git rev-parse origin/main)" || (echo "TS-C-v2 release Git differs from origin/main"; exit 2)
	SHAIWEI_TS_C_V2_RELEASE_GIT_HEAD="$(TS_C_V2_RELEASE_GIT_HEAD)" docker compose -f compose.ts-c-qualification-v2.yaml --profile ts-c-v2-fixture build ts-c-v2-fixture

docker-ts-c-v2-fixture: ## 真实画像前断网运行合成fixture与合同测试
	docker compose -f compose.ts-c-qualification-v2.yaml --profile ts-c-v2-fixture run --rm --no-deps ts-c-v2-fixture

docker-ts-c-v2-profile: ## 唯一一次v2密度画像（许可开启年口径，不读收益）
	mkdir -p data/research/trend_swing/ts-c-trigger-qualification-v2
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-c-qualification-v2.yaml --profile ts-c-v2-live run --rm --no-deps ts-c-v2-profile

docker-ts-c-v2-audit: ## 独立复核v2密度、许可开启年规则与门禁
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.ts-c-qualification-v2.yaml --profile ts-c-v2-live run --rm --no-deps ts-c-v2-auditor
...
docker-m6-production-head30-build: ## 以已推送实现构建M6-4B一次性Head30镜像；不运行真实效果
	@test -n "$(M6_PRODUCTION_HEAD30_RELEASE_GIT_HEAD)" || (echo "M6_PRODUCTION_HEAD30_RELEASE_GIT_HEAD is required"; exit 2)
	SHAIWEI_M6_PRODUCTION_HEAD30_RELEASE_GIT_HEAD="$(M6_PRODUCTION_HEAD30_RELEASE_GIT_HEAD)" docker compose -f compose.m6-production-head30-release.yaml --profile m6-production-head30 build m6-production-head30-runner
docker-m6-production-head30-fixture: ## 最终镜像内断网运行纯合成工程门；不挂载Qlib/M6 effect
	docker run --rm --network none --read-only --user "$$(id -u):$$(id -g)" --tmpfs /tmp:rw,noexec,nosuid,size=2g shaiwei:m6-production-head30-release-v1 python -m shaiwei.research.production_conversion.fixture --output-root /tmp/m6-production-head30-fixture
docker-m6-production-head30-run: ## 仅在精确scope获明确批准后运行唯一真实treatment runner
...
docker-m6-production-head30-price-recovery-build: ## 构建M6-4B-R2空成交价恢复镜像；不运行真实效果
	@test -n "$(M6_PRODUCTION_HEAD30_PRICE_RECOVERY_GIT_HEAD)" || (echo "M6_PRODUCTION_HEAD30_PRICE_RECOVERY_GIT_HEAD is required"; exit 2)
	SHAIWEI_M6_PRODUCTION_HEAD30_PRICE_RECOVERY_GIT_HEAD="$(M6_PRODUCTION_HEAD30_PRICE_RECOVERY_GIT_HEAD)" docker compose -f compose.m6-production-head30-price-recovery.yaml --profile m6-production-head30-price-recovery build m6-production-head30-price-recovery-fixture
docker-m6-production-head30-price-recovery-fixture: ## daemon创建容器并运行断网纯合成门
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.m6-production-head30-price-recovery.yaml --profile m6-production-head30-price-recovery run --rm --no-deps m6-production-head30-price-recovery-fixture
docker-m6-production-head30-price-recovery-run: ## 仅在R2新scope精确批准后运行唯一真实runner
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.m6-production-head30-price-recovery.yaml --profile m6-production-head30-price-recovery run --rm --no-deps m6-production-head30-price-recovery-runner
docker-m6-production-head30-price-recovery-audit: ## R2 runner成功后运行无Qlib挂载的独立auditor
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.m6-production-head30-price-recovery.yaml --profile m6-production-head30-price-recovery run --rm --no-deps m6-production-head30-price-recovery-auditor
docker-m6-production-head30-audit-recovery-build: ## 构建R3 auditor-only薄恢复镜像；不读真实effect
	@test -n "$(M6_PRODUCTION_HEAD30_AUDIT_RECOVERY_GIT_HEAD)" || (echo "M6_PRODUCTION_HEAD30_AUDIT_RECOVERY_GIT_HEAD is required"; exit 2)
	SHAIWEI_M6_HEAD30_AUDIT_RECOVERY_GIT_HEAD="$(M6_PRODUCTION_HEAD30_AUDIT_RECOVERY_GIT_HEAD)" docker compose -f compose.m6-production-head30-audit-recovery.yaml --profile m6-production-head30-audit-recovery build m6-production-head30-audit-recovery
docker-m6-production-head30-audit-recovery-fixture: ## 最终薄镜像内断网运行纯合成身份恢复门
...
docker-m6-production-head30-audit-output-root-recovery-build: ## 构建R7输出根薄恢复镜像；不读真实effect
	M6_PRODUCTION_HEAD30_AUDIT_OUTPUT_ROOT_RECOVERY_GIT_HEAD="$$(git rev-parse HEAD)"; SHAIWEI_M6_HEAD30_AUDIT_OUTPUT_ROOT_GIT_HEAD="$$M6_PRODUCTION_HEAD30_AUDIT_OUTPUT_ROOT_RECOVERY_GIT_HEAD" docker compose -f compose.m6-production-head30-audit-output-root-recovery.yaml --profile m6-production-head30-audit-output-root-recovery build m6-production-head30-audit-output-root-recovery-fixture
docker-m6-production-head30-audit-output-root-recovery-fixture: ## daemon断网验证真实audit宿主根写读哈希删除；不挂载effect
...
docker-m6-head30-500k-entrypoint-recovery-build: ## 构建M6-5B-R1入口恢复镜像；不读真实目标/价格/效果
	SHAIWEI_M6_HEAD30_500K_ENTRY_RECOVERY_GIT_HEAD="$$(git rev-parse HEAD)" docker compose -f compose.m6-head30-500k-entrypoint-recovery.yaml --profile m6-head30-500k-entrypoint-recovery build m6-head30-500k-entrypoint-recovery-fixture
docker-m6-head30-500k-entrypoint-recovery-fixture: ## 最终镜像断网穿过runner/auditor CLI和合成双跑
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.m6-head30-500k-entrypoint-recovery.yaml --profile m6-head30-500k-entrypoint-recovery run --rm --no-deps m6-head30-500k-entrypoint-recovery-fixture
docker-m6-head30-500k-entrypoint-recovery-run: ## 仅在R1新scope精确获批后运行一次真实50万元回放
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.m6-head30-500k-entrypoint-recovery.yaml --profile m6-head30-500k-entrypoint-recovery run --rm --no-deps m6-head30-500k-entrypoint-recovery-runner
docker-m6-head30-500k-entrypoint-recovery-audit: ## R1 runner成功后运行独立审计
	SHAIWEI_HOST_UID="$$(id -u)" SHAIWEI_HOST_GID="$$(id -g)" docker compose -f compose.m6-head30-500k-entrypoint-recovery.yaml --profile m6-head30-500k-entrypoint-recovery run --rm --no-deps m6-head30-500k-entrypoint-recovery-auditor
...
docker-web-control-build: ## Web 是双镜像组件；统一通过内容寻址 release 门构建，不允许单镜像旁路
	$(PYTHON) -m shaiwei.build_identity.web_release build
...
docker-web-build: ## 从已推送干净 HEAD 构建并验证双镜像 Web 候选；相同候选只复用不重建
	$(PYTHON) -m shaiwei.build_identity.web_release build
docker-web-release-prepare-successor: ## 显式归档当前已部署候选后，才允许构建后继双镜像候选
	$(PYTHON) -m shaiwei.build_identity.web_release prepare-successor
...
docker-web-release-promote: docker-web-control-init ## 首次受控提升、旧版回滚演练并重新提升同一候选
	$(PYTHON) -m shaiwei.build_identity.web_release promote
docker-web-up: docker-web-control-init ## 只启动已提升且身份通过的本机只读 Web release
	$(PYTHON) -m shaiwei.build_identity.web_release start
docker-web-status: ## 查看隔离 Web 服务状态
	$(PYTHON) -m shaiwei.build_identity.web_release status
