"""Frozen P4-0 source collection contract for 000698.SH."""
...
PROTOCOL_PATH = PROJECT_ROOT / "config" / "p4_star100_v1.yaml"
FIELDS = {
    "index_daily": (
        "ts_code",
        "trade_date",
...
class Star100CollectionError(RuntimeError):
    """Raised when provider evidence violates the frozen source contract."""
...
def canonical_frame_sha256(api_name: str, frame: pd.DataFrame) -> str:
    ...
    if missing := set(columns) - set(frame.columns):
        raise Star100CollectionError(f"{api_name} missing fields: {sorted(missing)}")
...
def validate_response(request: Request, frame: pd.DataFrame, row_limit: int) -> pd.DataFrame:
    ...
    if not frame.empty:
        ...
        if not dates.between(request.start_date, request.end_date).all():
            raise Star100CollectionError(f"{request.api_name} returned out-of-window dates")
        security_column = "con_code" if request.api_name == "index_weight" else "ts_code"
        if frame[security_column].astype("string").str.endswith(".BJ", na=False).any():
            raise Star100CollectionError(f"{request.api_name} returned forbidden .BJ row")
    return frame.loc[:, fields].copy()


class StableCollector:
    """Serial collector requiring two identical responses before immutable commit."""
