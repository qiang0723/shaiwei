"""Serial, immutable retrieval of official Star50 membership materials."""
...
class CurlFetcher:
    """Fetch official files with curl while enforcing the frozen v2 policy."""
    ...
    def _load_state(self) -> dict[str, dict[str, object]]:
        ...
        if payload.get("schema_version") != "p2-star50-official-retrieval-v2":
    ...
    def _save_state(self) -> None:
        payload = {
            "schema_version": "p2-star50-official-retrieval-v2",
    ...
    def _read_cached(self, url: str) -> tuple[bytes, SourceRecord] | None:
        ...
        record = SourceRecord(**{key: cached.get(key) for key in SourceRecord.__dataclass_fields__})
...
def _is_candidate(row: dict[str, str]) -> bool:
    title = row["title"]
    if title == "关于发布上证科创板50成份指数的公告":
        return True
    if not title.startswith("关于"):
        return False
    direct = "科创50" in title
    broad = "上证50、上证180、上证380" in title
    adjustment = "调整" in title and ("样本" in title or "定期调整结果" in title)
    return adjustment and (direct or broad)
...
def _attachment_urls(content: bytes, parent_url: str) -> list[str]:
    ...
    for anchor in soup.select("a[href]"):
        ...
        if ATTACHMENT_RE.search(href):
            urls.add(href)
    return sorted(urls)


def discover(fetcher: CurlFetcher, protocol: dict[str, object]) -> dict[str, object]:
    cutoff = str(protocol["source_policy"]["source_cutoff_date"]).replace("-", "")
    launch = str(protocol["initial_set_contract"]["publication_must_be_no_later_than"]).replace("-", "")
    launch = min(launch, "20200619")
    ...
    for page_number in range(1, 100):
        ...
        for row in rows:
            if launch <= row["announcement_date"] <= cutoff and _is_candidate(row):
                candidate_rows[row["source_url"]] = row
        if dates and max(dates) < launch:
            break
    else:
        raise OfficialFetchError("official archive discovery exceeded bounded page limit")

    page_records: list[SourceRecord] = []
    attachment_records: list[SourceRecord] = []
    for url, row in sorted(candidate_rows.items(), key=lambda item: (item[1]["announcement_date"], item[0])):
        content, record = fetcher.fetch(
            url,
            purpose="initial_publication" if url == protocol["source_policy"]["initial_publication_url"] else "membership_announcement",
            title=row["title"],
        )
        page_records.append(record)
        for attachment_url in _attachment_urls(content, url):
    ...
    payload = {
        "schema_version": "p2-star50-official-discovery-v2",
        "source_cutoff_date": protocol["source_policy"]["source_cutoff_date"],
        ...
        "sources": [
            asdict(record)
            for record in [*archive_records, *page_records, *attachment_records]
        ],
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if protocol.get("schema_version") != "p2-star50-protocol-v2":
        raise SystemExit("official fetch requires frozen p2-star50-protocol-v2")
    payload = discover(CurlFetcher(protocol), protocol)
    print(
        json.dumps(
            {
