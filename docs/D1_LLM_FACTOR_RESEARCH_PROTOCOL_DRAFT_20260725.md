...
| D1-2B | 40 次生成与发现期评价 | W1-W6/G1、追加批次 | 用户已批准D1总$10；本批独立真身固定40次/$1，先提交推送再运行 |
...
授权。以下原待确认项已由用户在 2026-07-25 明确批准并由独立执行协议接管；D1-2A 原文保留：

1. 首轮使用 V4 Pro thinking/high，而不是 Flash；
2. 40 次上限及每主题 4 独立 + 4 变异；
3. 原草案 `$0.75` 不回写；D1 总授权 `$10`，首批独立硬熔断 `$1`；
