...
- 六项门先验证生产release证据与冻结scope精确一致，再覆盖候选身份、真实 `starting`、共享合同收敛、守卫零回滚及生产证据哈希不变。
- claim 先于 fixture Docker 命令；成功或失败均写 report/tree/receipt，同 scope 不得重跑。
- 新 Compose 资产已进入构建身份注册表，注册覆盖由 97/97 提升为 98/98。
- 模块 381 行、测试 287 行，没有形成超大单文件。

## 验证

- R2D-R3A、release 与 build registry 专项：63 PASS；
...
- 全仓：1,945 PASS；架构门：13 PASS；17条warning均为既有第三方弃用或pandas未来行为提示。
