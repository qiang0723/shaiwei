...
- [ ] P0.5 模拟组合与前瞻绩效闭环（首个 FORWARD 待验收）：用户已于 2026-07-22 冻结初始资金 500,000 RMB；`paper-v1` 策略 SHA-256 `eaa341b5a3eee94347c7a8453a3e52f1986e3707abfbb6bb69a6d9298c320cc8`。纯函数成交/会计、公司行为、追加式账户/事件/运行账本、Docker 子流程和三组只读查询已实现。Docker 已将 20260717/20/21/22 四个真实执行日写入正式账本：首次调仓 30 个订单、22 笔成交、8 个整手门槛拒绝；7/20 入账 600460.SH 现金分红 32 元；四日账务恒等差均 0；重复运行 `NOOP` 且账本哈希不变。完成审计后又补入执行日不得晚于 UTC+8 当日、必须为信号后首个开市日、行情批次日期必须精确一致、公司行为实施公告不得晚于执行日和 `.BJ` fail-closed；模拟“产物已写且首条事件已追加后中断”可在下一次运行保留 FAIL 并幂等恢复为 PASS。独立 `paper-verify` 已从账户/事件/运行账本重放四日 150 个事件，逐项核对事件身份、载荷哈希、前态链、订单/成交/公司行为/持仓/现金/NAV、账户恒等和不可变产物，Docker 返回 PASS；scheduler 每轮模拟仓后自动执行该 verifier，损坏即进入 degraded。新运行产物显式固化 `execution_policy_version`；旧 BACKFILL 查询仅在当前配置策略哈希与产物一致时解析为 `paper-v1`，避免配置变更后伪报版本。当前受控代码快照为 `aede7ac0ee26dca96e1026c4e11ff17d02c4f51223072380676d312cbf7c9edd`，183 项全量测试与 Docker 16 项专项测试通过。历史四日仍一律为 `BACKFILL`，`forward_status=NOT_READY`；须取得首个 2026-07-23+ `FORWARD` 自然运行后才能勾选完成，证据见 `docs/P05_BACKFILL_ACCEPTANCE_20260722.md`。
