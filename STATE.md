...
## 当前阶段
阶段 0（基线）已完成；阶段 1 仅完成有界 GP→证据→G1 施工预演，正确 Top2 均 REJECT、正式因子库 0 插入。锁竞争修复后当前代码版本连续三次完整“信号 → 下一交易日开盘对账”已于 2026-07-22 完成 3/3，核心任务验收 PASS、通知通道 WARN；同日完成飞书通知健壮性修复。P0.5 模拟组合的工程、Docker 接入和四日 BACKFILL 已 PASS，当前只等待 2026-07-23+ 首个自然 `FORWARD` 结果。

验收后的结果路线已于 2026-07-22 更新为：先完成 P0.5 模拟组合结果闭环，再验证主力资金流对 Alpha158 的增量，随后以 `000688.SH` 建科创50独立策略臂；Web 在模拟仓和现有证据查询契约稳定后按 Grafana/MLflow/TradingView/Ant Design 主流范式施工只读首版。完整目标、输出、通过条件和禁止事项见 `docs/ROADMAP.md`。P0.5 首个自然前瞻结果未通过前，不将历史回放收益当成策略结论，也不启动 P1/P2。
...
- [ ] P0.5 模拟组合与前瞻绩效闭环（首个 FORWARD 待验收）：用户已于 2026-07-22 冻结初始资金 500,000 RMB；`paper-v1` 策略 SHA-256 `eaa341b5a3eee94347c7a8453a3e52f1986e3707abfbb6bb69a6d9298c320cc8`。纯函数成交/会计、公司行为、追加式账户/事件/运行账本、Docker 子流程和三组只读查询已实现。Docker 已将 20260717/20/21/22 四个真实执行日写入正式账本：首次调仓 30 个订单、22 笔成交、8 个整手门槛拒绝；7/20 入账 600460.SH 现金分红 32 元；四日账务恒等差均 0；重复运行 `NOOP` 且账本哈希不变。完成审计后又补入执行日不得晚于 UTC+8 当日、必须为信号后首个开市日、行情批次日期必须精确一致、公司行为实施公告不得晚于执行日和 `.BJ` fail-closed；模拟“产物已写且首条事件已追加后中断”可在下一次运行保留 FAIL 并幂等恢复为 PASS。独立 `paper-verify` 已从账户/事件/运行账本重放四日 150 个事件，逐项核对事件身份、载荷哈希、前态链、订单/成交/公司行为/持仓/现金/NAV、账户恒等和不可变产物，Docker 返回 PASS；scheduler 每轮模拟仓后自动执行该 verifier，损坏即进入 degraded。新运行产物显式固化 `execution_policy_version`；旧 BACKFILL 查询仅在当前配置策略哈希与产物一致时解析为 `paper-v1`，避免配置变更后伪报版本。新增 `paper-acceptance` 机器裁判：当前返回 `NOT_READY/replay_status=PASS`；首个 `FORWARD` 后将同时硬核代码/策略身份、Docker operator、新鲜度、`.BJ=0`、账本重放、飞书开始/完成 PASS 及证据哈希，scheduler 每轮自动执行。当前受控代码快照为 `261f58b858dbc46d49ffb9f623e8868dcb10891cc2dadd2292728da6de7eb4fa`，184 项全量测试与 Docker 17 项专项测试通过。历史四日仍一律为 `BACKFILL`，`forward_status=NOT_READY`；须取得首个 2026-07-23+ `FORWARD` 自然运行后才能勾选完成，证据见 `docs/P05_BACKFILL_ACCEPTANCE_20260722.md`。
...
## 待答点
- 后台结果路线调整为 P0 当前版本三次完整影子闭环 → 验收后健壮性复核 → P0.5 模拟组合与前瞻绩效闭环 → P1 资金流增量验证 → P2 科创50独立基线。P0.5 首批模拟仓查询现已稳定，Web 技术栈和原型可旁路评审；代码施工须另立目标，且不得阻塞或污染后台主线。
- P0.5 初始资金已冻结为人民币 50 万元，并同时输出归一化净值。50 万元下首个真实信号有 8 个目标因主板 100 股/科创板 200 股门槛无法买入；这是实际账户约束结果，不允许用碎股或目标权重补齐。当前只待 2026-07-23+ 首个自然 `FORWARD` 验收，不把四日 BACKFILL 净值用于策略裁决。
