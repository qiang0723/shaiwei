...
## 当前阶段
阶段 0（基线）已完成；阶段 1 仅完成有界 GP→证据→G1 施工预演，正确 Top2 均 REJECT、正式因子库 0 插入。前瞻影子仍等待连续三个真实交易日验收。

2026-07-16 已确认验收后的结果路线：主力资金流只作为候选数据做 Alpha158 之上的增量验证；科创50以 `000688.SH` 建独立策略臂，不照搬中证800 Top30/历史窗口；Web 在指标与查询契约稳定后按 Grafana/MLflow/TradingView/Ant Design 主流范式施工只读首版。完整目标、输出、通过条件和禁止事项见 `docs/ROADMAP.md`。三日影子未满前不得启动上述生产改造。
...
- [ ] 前瞻影子真实运行验收（1/3）：继续等待后续两个真实交易日，验证次日开盘可成交率、换手、开盘偏差、预计成本、异常恢复和零重复/零人工修数；未满三日不得宣称闭环完成。
...
## 待答点
- 结果路线图已冻结为 P0 三日影子验收 → P1 资金流增量验证 → P2 科创50独立基线 → P3 专业只读 Web；当前仅 P0 可推进，后续不得绕过 `docs/ROADMAP.md` 的启动条件。
