...
## 当前阶段
阶段 0（基线）· Day 1-5 真实采集中 — 代码层、证据门和失败即停自动流已收口；基础表已完成，停复牌断点已推进至 2019-02-18，尚未完成全量采集与 G0 实测。
...
- [x] 真实首跑边界与吞吐修复：Tushare 合法空响应规范化为冻结 schema 的 0 行批次；1 万积分按全局 0.15 秒请求起点间隔+8 路在途请求隐藏网络延迟，硬顶仍为 400 次/分钟，账本仍串行有序提交
...
- [x] 离线质量门：77 项测试、Ruff、compileall、pip check、账本追加约束通过（2026-07-15；不等同真实数据验收）
- [x] Day 1-2 实测：trade_cal / stock_basic（含退市）/ CSI300+CSI800 index_weight 历史共 261 批已提交；bootstrap 可按账本全量跳过
