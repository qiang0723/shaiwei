...
## 当前阶段
阶段 0（基线）已完成；阶段 1 仅完成有界 GP→证据→G1 施工预演，正确 Top2 均 REJECT、正式因子库 0 插入。锁竞争修复后当前代码版本连续三次完整“信号 → 下一交易日开盘对账”已于 2026-07-22 完成 3/3，核心任务验收 PASS、通知通道 WARN；同日已完成验收后飞书通知健壮性修复与全量回归，P0.5 模拟组合施工入口打开。

验收后的结果路线已于 2026-07-22 更新为：先完成 P0.5 模拟组合结果闭环，再验证主力资金流对 Alpha158 的增量，随后以 `000688.SH` 建科创50独立策略臂；Web 在模拟仓和现有证据查询契约稳定后按 Grafana/MLflow/TradingView/Ant Design 主流范式施工只读首版。完整目标、输出、通过条件和禁止事项见 `docs/ROADMAP.md`。当前版本三次完整影子闭环未满前不得启动上述生产改造。

2026-07-19 用户明确后台仍为主线，同时授权 Web 方案旁路持续优化。P0 验收期间仅形成文档、指标草案、查询契约草案和低保真线框，不写 Web/后端代码、不启动服务、不改变生产口径；按 2026-07-22 更新后的顺序，P0.5 首批查询契约稳定后才进入 Web 代码施工。初版方案见 `docs/WEB_DESIGN.md`。

2026-07-22 已确认 Web 设计协作方式：主线负责指标与证据口径裁决，Dashboard 架构能力负责信息层级与下钻，Quant Visualization 负责量化图表，Figma UI/UX 负责视觉与原型，浏览器 QA 负责竞品核对、响应式与交互验收；P0.5 首批查询契约稳定后才引入前端实现角色。Figma 等外部工具不得承载真实数据、密钥或不可变证据，设计真身仍须导出并保存在本仓库。

2026-07-22 用户授权在 P0 后增加 P0.5“模拟组合与前瞻绩效闭环”，并将其置于资金流验证之前。首版只运行正式模型基准仓：消费不可变信号，以信号后下一交易日官方开盘作为唯一成交时点，持续记录订单、成交、实际持仓、现金、成本和每日净值；无法成交时不得把目标权重冒充实际持仓。未来 LLM 主观研判若启用，必须进入独立账户并与模型基准仓并行比较。P0 三次闭环和验收后健壮性复核已完成，可以开始施工；首次正式模拟前仍须冻结初始资金等执行口径，详见 `docs/PAPER_PORTFOLIO_SPEC.md`。

2026-07-16 已记录长期定位：LLM 因子挖掘将作为筛微未来的主要研究工作，但不是生产策略控制者；输入不局限于观象，还包括筛微本地数据、论文/规则/研报/网页、开源代码、经授权的新数据源和历史实验记忆。当前不施工常设系统；待当前版本三次真实闭环完成且验收后健壮性欠账清零，再按 `docs/LLM_RESEARCH.md` 讨论总体架构、知识雷达、观象只读桥、代码沙箱、实验纪律和日常运行节奏。
...
- [ ] P0.5 模拟组合与前瞻绩效闭环（施工中）：用户已于 2026-07-22 冻结初始资金 500,000 RMB；`paper-v1` 同日冻结，策略 SHA-256 `eaa341b5a3eee94347c7a8453a3e52f1986e3707abfbb6bb69a6d9298c320cc8`。纯函数成交/会计、公司行为、追加式账户/事件/运行账本、Docker 子流程和三组只读查询已实现；10 项专项测试与四个真实执行日零写入预演通过。预演结果为 7/17 首次调仓 30 个目标、22 笔成交、8 个因整手门槛拒绝，日末 22 个实际持仓、现金 180,525.98 元、净资产 477,333.69 元；7/20 正确入账 600460.SH 现金分红 32 元；四日账务恒等差均 0。上述一律为 `BACKFILL`，尚未写正式账本；须先提交 schema 基线，再由 Docker 追加回放产物并完成双跑幂等和首个 2026-07-23+ `FORWARD` 自然运行验收。
...
## 待答点
- 后台结果路线调整为 P0 当前版本三次完整影子闭环 → 验收后健壮性复核 → P0.5 模拟组合与前瞻绩效闭环 → P1 资金流增量验证 → P2 科创50独立基线。Web 方案设计可旁路推进；Web 代码须等待 P0.5 首批模拟仓与现有证据查询契约稳定，且不得阻塞或污染后台主线。
- P0.5 初始资金已冻结为人民币 50 万元，并同时输出归一化净值。50 万元下首个真实信号有 8 个目标因主板 100 股/科创板 200 股门槛无法买入；这是实际账户约束结果，不允许用碎股或目标权重补齐。
