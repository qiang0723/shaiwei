...
## 2026-07-26 · G8-1 采集工程实现完成，待真实 Docker 执行
...
- 宿主全仓 314 PASS，Ruff/compileall/Compose/diff-check PASS。尚未运行真实采集；必须先提交实现并以该提交
  构建镜像，再执行两遍真实幂等验收。
...
- 证据包只落项目 `data/g8/fund_evidence`，追加账本只记相对路径、哈希、行数和状态，不记净值/金额/
  备注原文或凭据。
- 本协议尚未实现、未运行真实采集。即使后续全部 PASS，也只允许进入管理人 HTTPS 交叉核验与
  费率谱系阶段；G8 保持 `NOT_READY`。
...
- G8 公式已冻结，G8-0 已确认证监会电子披露站可覆盖六只产品的净值、分红备注和
  法定文件发现，但其稳定机器入口仅为 HTTP。六只产品的不可覆盖采集器、管理人 HTTPS 交叉核验、
  费率有效期谱系和裁决账本尚未施工；完成前不得升为 `VERIFIED`、构造总收益或运行 G8，也不能以聚合
  平台数据冒充权威源。
