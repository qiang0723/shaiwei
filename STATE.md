...
## 当前阶段
阶段 0（基线）已完成；阶段 1 仅完成有界 GP→证据→G1 施工预演，正确 Top2 均 REJECT、正式因子库 0 插入。前瞻影子正在做锁竞争修复后当前代码版本连续三次完整“信号 → 下一交易日开盘对账”验收，当前完成 2/3。
...
- [ ] 前瞻影子当前版本完整验收（2/3）：`20260721` 信号于 `20260722` 对账为第 3 次；正式验收最早为 2026-07-22。仍须核验整日 PASS、原始批次无 `.BJ`、飞书开始/完成、信号准时、次日可成交率/换手/开盘偏差/预计成本、重复运行幂等、零人工修数、异常恢复、Git 脱敏与远端同步；不得仅凭当前 `trial_ready` 日期集合判定宣称完成。
- [ ] P0.5 模拟组合与前瞻绩效闭环：方案已授权、代码未启动；启动门为 P0 3/3 完整通过且验收后健壮性欠账复核完成。首版须先冻结初始资金、费用、交易单位、公司行为和成交失败语义，再实现追加式订单/成交/持仓/现金/净值账本及只读查询契约。
...
- 2026-07-21 飞书“日增量开始”出现一次网络超时，随后 4 次投递自行恢复；冻结期不因一次性外部抖动修改代码。P0 完成后的健壮性复核须评估有界重试、重复消息风险和恢复状态留痕，不能回写或删除本次 FAIL。
