...
## 2026-08-02 · M3-0科创板规则型PIT研究池协议已结果前冻结

- 施工前发现 `namechange` 最近采集日为2026-07-15，早于固定截止日；在任何成员构造前追加冻结
  源新鲜度门：`stock_basic`及科创板范围逐票`namechange`最近入账均须覆盖2026-07-31，截止日后
  生效事件仍不得使用。该补充只修源证据边界，不改变任何股票池规则或门槛。
- 已按 M1-0 边界另立 `m3-star-custom-pit-data-rule-gate-v1`，冻结科创板全市场/中盘/小盘三个
  `CUSTOM_RULE_BASED` 研究池的身份、月末形成/次日生效、上市满12个月、ST/退市逐日硬退出、
  60日流动性、20日市值及确定性三等分规则；不得冒充科创100、科创200或官方“科创300”。
- 结构可用门在成员构造前固定为全市场至少60只、中盘/小盘各至少20只及至少36个连续合格形成月；
  真实成员数量和覆盖尚未运行。当前只授权数据与规则可行性门，不调用DeepSeek、不计算因子或效果、
  不运行qlib/模型/回测/信号，生产授权仍为none。
