...
- [x] 六窗口 Alpha158+LightGBM 已真实完成：基准成本 5/6 窗口正超额，+50% 成本合并超额 +49.23%，两项 G0 回测条件均通过；影子步骤正确拒绝了与当前快照不匹配的哨兵报告。根因是旧代码哈希包含 Git HEAD，纯账本提交也会误使代码快照漂移；现改为受控文件内容哈希，显式排除 append-only 账本与信号
