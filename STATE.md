...
## 2026-08-03 · F2-1中证800基本面动态六候选效果协议已结果前冻结

- F2-0R数据门GO后，F2-1固定六项既登记公式与方向、2016H2—2018发现、2019—2024六个OOS窗、
  三压力期、三成本情景和`90% Alpha158 + 10%动态基本面正式残差`；不训练模型、不改主策略。
- 输入绑定F2-0R面板/报告/manifest、Alpha158预测、qlib内容树与暴露源账本；冻结前未读取任何F2
  标签关联、RankIC、收益、回撤、选股、持仓或G1结果。
- 多重检验不重置：F1静态六次与F2动态六次保留各自研究身份，G1必须合并两个登记家族，完整运行后
  累计`N=12`；窗口、成本和诊断不机械计成独立尝试。协议见
  `docs/F2_CSI800_FUNDAMENTAL_EFFECT_PROTOCOL_20260803.md`。
- 当前只完成结果前冻结，尚未构建残差或运行效果；须先推送本协议，再实现共享引擎的显式家族聚合与
  F2独立入口。不得提前看效果、追加第七候选、改方向/权重/窗口/成本或接生产。
- 结果前提交`cb17b3e`已先行推送；随后以显式runtime复用F1残差/组合/证据核心，新增F2独立合同与
  入口，G1默认保持单家族并只在F2证据显式声明时聚合F1+F2。定向17 PASS、全仓537 PASS，尚未
  构建F2残差或读取效果；实现提交仍须先推送后才能运行。
