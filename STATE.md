...
## 2026-08-12 · TS-1A-R4结果盲回调状态协议已冻结，尚未执行

- R4唯一状态口径冻结为：上一完整周VWAP下4%触达，随后同周收盘高于前一有效日高点且为阳线确认，
  下一官方开盘须在结构止损以上、周锚点及以下，止损距离严格小于15%；每票每周最多一次确认且非法
  开盘不重试。月低点、两周波动/振幅和结构止损上移也形成单一口径，不再复用v2.1 ATR规则。
- 结果前样本门固定为2019—2024真正合法事件至少60条、至少40个不同信号日、每年不少于3条且至少
  四年各不少于8条，Alpha158事件键覆盖100%；画像后不得放宽。
- H00906全收益基准须有可审计身份和完整区间；项目内缺失则`BLOCKED_BENCHMARK_DATA`，不得以
  `000906.SH`价格指数或本地代理替代。
