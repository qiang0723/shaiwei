...
- D1-2B 已完成 40/40 和发现期机械 Top2，实际费用 `$0.076626207`，未用预算不自动授权新批。下一步只能另立 D1-3：先做 Top2 人工经济解释/泄漏审查并结果前冻结，再决定是否运行不变 `g1-v1`；不通过即停止，不递补、不调门槛、不追加候选。任何阶段都不自动进入生产，观象桥、常驻知识雷达、程序级因子和新数据继续后置。
