...
2026-07-16 已确认验收后的结果路线：主力资金流只作为候选数据做 Alpha158 之上的增量验证；科创50以 `000688.SH` 建独立策略臂，不照搬中证800 Top30/历史窗口；Web 在指标与查询契约稳定后按 Grafana/MLflow/TradingView/Ant Design 主流范式施工只读首版。完整目标、输出、通过条件和禁止事项见 `docs/ROADMAP.md`。当前版本三次完整影子闭环未满前不得启动上述生产改造。

2026-07-19 用户明确后台仍为主线，同时授权 Web 方案旁路持续优化。P0 验收期间仅形成文档、指标草案、查询契约草案和低保真线框，不写 Web/后端代码、不启动服务、不改变生产口径；P0 通过且首批查询契约稳定后才进入代码施工。初版方案见 `docs/WEB_DESIGN.md`。

2026-07-22 已确认 Web 设计协作方式：主线负责指标与证据口径裁决，Dashboard 架构能力负责信息层级与下钻，Quant Visualization 负责量化图表，Figma UI/UX 负责视觉与原型，浏览器 QA 负责竞品核对、响应式与交互验收；P0 通过后才引入前端实现角色。Figma 等外部工具不得承载真实数据、密钥或不可变证据，设计真身仍须导出并保存在本仓库。
...
## 待答点
- 后台结果路线仍为 P0 当前版本三次完整影子闭环 → P1 资金流增量验证 → P2 科创50独立基线；Web 方案设计可旁路推进，但代码施工必须等待 P0 通过和首批查询契约稳定，且不得阻塞或污染后台主线。
- 2026-07-21 飞书“日增量开始”出现一次网络超时，随后 4 次投递自行恢复；冻结期不因一次性外部抖动修改代码。P0 完成后的健壮性复核须评估有界重试、重复消息风险和恢复状态留痕，不能回写或删除本次 FAIL。
- LLM 持续因子研究线已记为 D1 方案评审门：P0 完整通过且验收后健壮性欠账清零时再具体讨论，不自动实施、不自动重排 P1-P3。届时必须逐项裁决多源范围、公开框架复用边界、观象平台独立决策、PIT知识时钟、AST/动态截断安全门、固定预算与常设化条件。
