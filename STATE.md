...
## 2026-08-23 · M6-5C-A退市风险退出方法结果前冻结

- 官方事实确认`002505.SZ`在2024-06-04至07-02连续20个交易日收盘低于1元，07-03起已无普通
  市场退出窗口且不进入退市整理期；该轮连续低价风险最迟于06-19已有公开风险提示，早于封存目标
  06-20执行。故拒绝退市日按零价/最后价结算，也拒绝删股后向其余目标再分配权重。
- M6-5C-A采用独立事前风险退出层：信号日收盘可知的连续10个有效交易收盘严格低于1元即阻止新买，
  已持仓退出意图锁存到真实成交；卖不出不造fill/cash，触及退市仍无处置则继续BLOCKED。该方法由
  已知W6失败触发，未来历史结果只能标`POST_HOC_METHOD_RECOVERY_DIAGNOSTIC`，新尝试家族与旧2次
  尝试分离。当前仅冻结ADR、官方证据、配置和协议；不读真实目标/价格/效果，不写账本，不改
  `paper-v1`、Web或scheduler。下一步只做纯状态机与synthetic门。
