...
## 2026-08-01 · Web页面模块化第二阶段协议已冻结，尚未施工

- 因子页952行、实验页861行，均把路由、查询、筛选、证据格式化和复杂展示混在单文件；本目标只按
  目录/详情/比较或历史/展示原语拆分，两个入口收窄为100行内路由，所有新增模块不超过600行。
- HTTP契约、查询参数、文案、ARIA、CSS类名、图表口径和既有E2E全部冻结；不修改后台、Compose、
  scheduler、Top20候选或8月3日守护，不升级或联网安装依赖。
- 本节仅证明结果前结构协议已冻结；代码、Docker/浏览器回归和终版验收尚未完成。见
  `docs/WEB_PAGE_MODULARIZATION_PROTOCOL_20260801.md`。
