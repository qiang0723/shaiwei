...
## 当前阶段
阶段 0（基线）已完成；阶段 1 仅完成有界 GP→证据→G1 施工预演，正确 Top2 均 REJECT、正式因子库 0 插入。锁竞争修复后当前代码版本连续三次完整“信号 → 下一交易日开盘对账”已于 2026-07-22 完成 3/3，核心任务验收 PASS、通知通道 WARN；当前进入验收后健壮性复核。
...
2026-07-22 用户授权在 P0 后增加 P0.5“模拟组合与前瞻绩效闭环”，并将其置于资金流验证之前。首版只运行正式模型基准仓：消费不可变信号，以信号后下一交易日官方开盘作为唯一成交时点，持续记录订单、成交、实际持仓、现金、成本和每日净值；无法成交时不得把目标权重冒充实际持仓。未来 LLM 主观研判若启用，必须进入独立账户并与模型基准仓并行比较。P0 当前三次闭环和验收后健壮性复核完成前只写设计文档，不施工代码或服务，详见 `docs/PAPER_PORTFOLIO_SPEC.md`。
...
- [ ] 验收后健壮性复核：飞书在 2026-07-21“日增量开始”和 2026-07-22“日增量完成”连续两个交易日各发生一次网络超时，均由后续真实投递证明恢复。须评估有界重试、幂等消息身份、重复消息风险和恢复状态留痕，并在不改核心任务退出语义的前提下完成回归。
- [ ] P0.5 模拟组合与前瞻绩效闭环：方案已授权、代码未启动；启动门为验收后健壮性欠账修复并回归通过。首版须先冻结初始资金、费用、交易单位、公司行为和成交失败语义，再实现追加式订单/成交/持仓/现金/净值账本及只读查询契约。
...
- P0.5 首次运行前仍须冻结模拟初始资金。建议候选为人民币 100 万元并同时输出归一化净值；该数影响整手和最低费用，未冻结前不得生成可比较的正式模拟业绩。
- 飞书连续两个交易日各有一次网络超时：2026-07-21“日增量开始”失败后 4 次 PASS，2026-07-22“日增量完成”失败后 3 次 PASS。原始 FAIL 均须保留；健壮性复核须评估有界重试、幂等消息身份、重复消息风险和恢复状态留痕，不能把补发伪装成原始事件成功。
