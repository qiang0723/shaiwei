...
## 当前阶段
阶段 0（基线）已完成；阶段 1 仅完成有界 GP→证据→G1 施工预演，正确 Top2 均 REJECT、正式因子库 0 插入。锁竞争修复后当前代码版本连续三次完整“信号 → 下一交易日开盘对账”已于 2026-07-22 完成 3/3，核心任务验收 PASS、通知通道 WARN；同日完成飞书通知健壮性修复。P0.5 模拟组合的工程、Docker 接入、四日 BACKFILL 和 2026-07-23 首个自然 `FORWARD` 已全部 PASS，当前进入持续前瞻观察。

验收后的结果路线已于 2026-07-22 更新为：P0.5 模拟组合结果闭环完成后，验证主力资金流对 Alpha158 的增量，随后以 `000688.SH` 建科创50独立策略臂；Web 在模拟仓和现有证据查询契约稳定后按 Grafana/MLflow/TradingView/Ant Design 主流范式施工只读首版。完整目标、输出、通过条件和禁止事项见 `docs/ROADMAP.md`。P0.5 已通过，但一天前瞻观察不作策略结论；P1、Web 施工和 D1 方案评审均须另立目标。
...
- P0.5 初始资金人民币 50 万元的首个自然 `FORWARD` 已通过，后续由 scheduler 持续追加。50 万元下首个真实信号有 8 个目标因主板 100 股/科创板 200 股门槛无法买入；这是实际账户约束结果，不允许用碎股或目标权重补齐。当前仅有 1 个前瞻账户日，不把四日 BACKFILL 或一天 FORWARD 净值用于策略裁决。
- P1 资金流数据与特征准备已于 2026-07-24 完成：长期主源仍为 `tushare.moneyflow`，`moneyflow-pit-v1` 固定下一交易日可用；年度检查点结论已由 2,563 个官方交易日全量审计替代。严格单日门发现 46 个稳定失败日，按 `moneyflow-quality-v2` 整日隔离后期间覆盖 PASS；六候选原始面板、核心残差、W1—W6 Alpha158 增量残差和基线预测均已哈希绑定。工具仍隔离在 `tools/p1_moneyflow/`，生产快照保持 `261f58...`。下一步只运行已冻结的六候选同预算 W1—W6/成本/压力期比较并逐项调用 `g1-v1`；在完成前不得增加变体、修改门槛或接入生产。详见 `docs/MONEYFLOW_DATA_SPEC.md`、`docs/P1_MONEYFLOW_EXPERIMENT_PROTOCOL_V1.md` 与 `docs/P1_MONEYFLOW_FEATURE_ACCEPTANCE_20260724.md`。
