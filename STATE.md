...
- [x] 离线质量门：81 项测试、Ruff、compileall、pip check、账本追加约束通过（2026-07-15；不等同真实数据验收）
...
- [ ] Day 3-4：全量 daily+adj_factor+daily_basic（当前安全断点 3,426/24,792 批、三源各 1,142、4,183,535 行）→ 后复权 → 量纲统一 → 完整性对账 → 全部哨兵；当前真实断点 S4/S7 预跑 PASS，S7 修正 3 只股票 11 个有证据的源因子补丁、未归因有效跳变 0
