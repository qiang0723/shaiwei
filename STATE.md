...
结果路线现为：P0.5 持续积累真实前瞻观察；P1 首批六个简单资金流候选已全部 REJECT 且停止本家族追加变体；生产 scheduler 与开发工作树的发布快照隔离已于 2026-07-24 完整 PASS。P2-0 的 `p2-star50-protocol-v1` 永久保留 NO-GO：Tushare 首份权重按 T+1 仅能从 2020-08-03 生效，冻结起点缺 7 个交易日且无历史版本/修订字段。`p2-star50-protocol-v2` 以官方首批名单和全量调整公告重建 `000688.SH` 成员谱系；1,456 个交易日每日均为 50 只，和 72/72 个 Tushare 月度集合完全一致，官方谱系数据门 GO。P2-1 已在任何真实策略效果查看前完成独立真实数据集、动态 instruments、隔离 qlib 和 synthetic Alpha158/LightGBM/TopK/backtest 工程闭环，机器结论 `engineering_complete=true`、`strategy_results_inspected=false`、`strategy_effective=NOT_EVALUATED`、`production_authorization=none`。该 GO 只表示工程可运行，正式效果必须由主控另立 P2-2；中证800继续是唯一生产主策略。Web 可继续视觉与信息架构旁路设计；真实 API、Docker profile 和生产代码施工仍须另立目标，不能因隔离门通过而自动开工。完整目标、输出、通过条件和禁止事项见 `docs/ROADMAP.md`。
...
## 待答点
- P2 已形成三层不互相覆盖的裁决：v1 永久 NO-GO 证明 Tushare 不能单独充当 PIT 真身；v2 官方成员谱系数据门 GO 证明 2020-07-23 起的数据可行；P2-1 工程 GO 证明独立数据集、qlib 和 synthetic 通路可运行。三者均不证明策略有效。若主控决定继续，须另立 P2-2 并在任何真实效果运行前绑定终版 manifest/报告哈希；不得沿用工程 GO 自动训练、回测、看结果或接生产。
- 后台结果路线已完成 P0 → P0-R → P0.5 → P1 → 生产/开发发布隔离 → P2-0 → P2-1。P1 六候选均 REJECT，本家族停止追加变体；P2 当前停在工程门 GO，真实 IC、收益、回撤、排名、选股和信号仍未生成或查看。Web 技术栈和视觉原型可旁路评审，真实 API/Docker 代码仍须另立目标。
