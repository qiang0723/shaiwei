...
## 2026-08-01 · M1-0多股票池因子研究底座协议已冻结，尚未施工

- 用户确认筛微不能长期停留在中证800单股票池/单策略，需要把等待前瞻数据的时间用于科创50、
  科创100、科创200及规则型科创板股票池的因子研究；当前先建设机器可校验的多股票池底座。
- 首批登记中，中证800保持唯一既有生产策略；科创50允许未来另立新因子协议，但P2旧基线REJECT
  不改；科创100官方臂仍被历史成员谱系阻断；科创200、科创综指和三类自建科创板PIT池只允许
  数据/规则门，不得提前计算因子或冒充官方指数。
- M1-0只冻结股票池身份、PIT等级、允许用途和因子跨池评价合同，不调用DeepSeek、不读取结果、
  不运行G1/模型/回测，不修改scheduler、Top20候选或8月3日守护。见
  `docs/M1_MULTI_UNIVERSE_FOUNDATION_PROTOCOL_20260801.md`。
