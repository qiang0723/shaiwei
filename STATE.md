...
- [x] P2-2 科创50正式历史效果裁决（2026-07-25，NO-GO/REJECT）：任何真实 handler/model/backtest 前提交并推送 `ed5b1b0`，绑定 P2-1 终版、v2 五项、三份数据和 qlib 内容树/build identity 精确哈希，并为 P2-1 旧账本时间字段补充不改旧行的审计附录。唯一运行采用 Alpha158/LightGBM seed42、Top10/n_drop2、10 日调仓、1 亿元、次开盘和冻结成本；三窗 242/242/243 日、各 25 次调仓，基础净超额 -4.85%/-32.73%/-7.74%，727 日 pooled 基础/2x/额外滑点为 -50.40%/-51.98%/-51.79%，正超额 0/3；W1/W2 与 microcap_2024/volume_price_2026h1 回撤超过 20%。CSI800 无合法同期日收益+持仓真身，分散化门 `NOT_EVALUABLE`，不造代理。确定性复核 54/54 内容一致，终版入口再调用四项哈希不变且账本各 1 行；`historical_effect_gate=NO_GO`、`strategy_effective=REJECT`、`production_authorization=none`。基线停止，不调门槛、不追加变体，证据见 `docs/P2_STAR50_EFFECT_ACCEPTANCE_20260725.md`。
...
## 待答点
- P2 已形成四层不互相覆盖的裁决：v1 永久 NO-GO 证明 Tushare 不能单独充当 PIT 真身；v2 官方成员谱系数据门 GO 证明 2020-07-23 起的数据可行；P2-1 工程 GO 证明独立数据集、qlib 和 synthetic 通路可运行；P2-2 对冻结历史基线给出 `NO_GO/REJECT`。不得把 P2-2 表述成前述数据/工程门失败，也不得以其结果调门槛追加变体。
- 后台结果路线已完成 P0 → P0-R → P0.5 → P1 → 生产/开发发布隔离 → P2-0 → P2-1 → P2-2。P1 六候选和 P2 冻结历史基线均 REJECT 并停止；P2 不进入前瞻观察或生产。Web 技术栈和视觉原型可旁路评审，真实 API/Docker 代码仍须另立目标。
