...
## 2026-07-25 · D1-2B 首份响应后控制流恢复待执行

- `d1-llm-dsl-v1-batch-001` 已取得第 1/40 份完成响应，估算费用
  `0.0013746 USD`；解析、DSL 沙箱和冻结发现期评估通过。
- 第 2 个请求发出前因独立尝试反馈控制流缺陷 fail closed；无重复请求、无
  计费不确定性，生产 scheduler 未受影响。
- 纠错范围仅限“独立尝试忽略历史反馈”和“部分批次从下一缺失序号恢复”；
  第 1 份响应及三份账本前缀、四类忽略区产物均按哈希永久保留。
- 恢复附录：
  `config/d1_llm_factor_execution_recovery_v1.yaml`；
  说明：`docs/D1_LLM_FACTOR_EXECUTION_RECOVERY_20260725.md`。
- 当前仍是 D1 发现期研究，不运行 W1–W6、压力期、G1 或生产信号。
...
- [~] D1-2B 首批真实生成结果前冻结（2026-07-25，AUTHORIZED_NOT_RUN）：用户批准 D1 总额度 `$10` 并指令开工；本批独立真身固定 40 个完成响应和 `$1` 熔断，未用余额不自动授权后续批次。只读 2016-06-01—2018-12-31 发现期以支持后四次同主题反馈，W1—W6、压力期、G1、前瞻和生产全部禁读/禁跑。预调用官方复核发现 D1-2A 把版本名 `DeepSeek-V4-Pro` 与响应 `model=deepseek-v4-pro` 混为一字段，已在任何付费响应前分列更正；旧记录不回写。旧 v1 账本受 append-only 门保护并保持字节不变，D1-2B 使用 v2 账本。430,835,211 bytes / 54,464 文件 qlib 树重哈希一致，发现期 465,116 个可评价基础行和固定示例 631 个日 IC 均打通；当前真实 API、真实候选和费用仍为 0。冻结协议见 `docs/D1_LLM_FACTOR_EXECUTION_PROTOCOL_20260725.md`。
...
- 后台结果路线已完成 P2-2C 权威纠错并停止该基线。P1 六候选和 P2-2C 均保持 REJECT；P2 不进入前瞻观察或生产，不调参、不追加变体。P3-0 查询底座与 P3-1 三个正式页面均已 GO，本机回环首版可用；先积累真实使用证据，后续因子工厂、模型/回测、数据质量和系统运行页面必须先补各自只读查询契约并另立目标，不反向改后台口径。
...
- D1-2B 已获模型、40 次和预算授权并完成结果前施工，尚未发出付费请求。下一步必须先提交推送冻结真身，再建立独立真实研究发布镜像、做不回显 secret 存在性门、TLS 出口和输入哈希复核，随后串行生成并只做发现期评价；本批实际累计或下一次最坏预留超过 `$1` 即停，D1 总 `$10` 未用余额不自动开新批。D1-3 人工闸和 G1 再单独授权；任何阶段都不自动进入生产，观象桥、常驻知识雷达、程序级因子和新数据继续后置。
