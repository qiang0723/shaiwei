...
- P1 资金流数据可用性与 PIT 阶段已于 2026-07-23 完成：长期主候选冻结为 `tushare.moneyflow`，2016—2026 共 12 个年度检查点、50,252 条最新记录全部通过质量门，最低日覆盖 99.6465%，双快照未见即时修订；THS/DC 仅作短历史交叉诊断。`moneyflow-pit-v1` 固定下一交易日可用，结论为 `GO_DATA_ONLY`，只授权另立目标开展隔离增量因子实验。工具固定在 `tools/p1_moneyflow/`，生产代码快照保持 `261f58...`，不改模型、信号、门禁或 scheduler。详见 `docs/MONEYFLOW_DATA_SPEC.md` 与 `docs/P1_MONEYFLOW_DATA_QUALITY_20260723.md`。
