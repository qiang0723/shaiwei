...
## 当前阶段
阶段 0（基线）已完成；阶段 1 仅完成有界 GP→证据→G1 施工预演，正确 Top2 均 REJECT、正式因子库 0 插入。前瞻影子正在做锁竞争修复后当前代码版本连续三次完整“信号 → 下一交易日开盘对账”验收，当前完成 1/3。
...
- [ ] 前瞻影子当前版本完整验收（1/3）：`20260720` 信号于 `20260721` 对账为第 2 次，`20260721` 信号于 `20260722` 对账为第 3 次；正式验收最早为 2026-07-22。每次均须核验整日 PASS、原始批次无 `.BJ`、飞书开始/完成、信号准时、次日可成交率/换手/开盘偏差/预计成本、重复运行幂等、零人工修数、Git 脱敏与远端同步；不得仅凭当前 `trial_ready` 日期集合判定宣称完成。
