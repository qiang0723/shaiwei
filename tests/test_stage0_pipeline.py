...
def test_stage0_pipeline_has_fail_closed_order_and_no_stage1():
    ...
    assert names == [
        ...
        "industry_membership", "market", "financial", "crosscheck",
...
def test_pipeline_resume_is_bound_to_asof_and_code_hash(tmp_path: Path):
    ...
    events = [
        {"step": "quality", "status": "PASS", "as_of": "2026-07-15", "code_snapshot_sha256": "a", "data_snapshot_sha256": "d"},
        {"step": "runtime", "status": "FAIL", "as_of": "2026-07-15", "code_snapshot_sha256": "a", "data_snapshot_sha256": "d"},
        {"step": "market", "status": "PASS", "as_of": "2026-07-15", "code_snapshot_sha256": "b", "data_snapshot_sha256": "d"},
    ]
    log.write_text("".join(json.dumps(event) + "\n" for event in events), encoding="utf-8")
    assert _completed_steps(log, code_hash="a", data_hash="d", as_of=date(2026, 7, 15)) == {"quality"}
    assert _completed_steps(log, code_hash="a", data_hash="new", as_of=date(2026, 7, 15)) == set()
