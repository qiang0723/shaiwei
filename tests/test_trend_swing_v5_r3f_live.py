...
from shaiwei.research.trend_swing.v5_r3f_live import run_batch, run_preflight
...
from shaiwei.research.trend_swing.v5_r3f_result_audit import audit_batch
from shaiwei.research.trend_swing.v5_response_contract import CONTRACT_SHA256 as TERMINAL_SHA
from shaiwei.research.trend_swing.v5_transport import BUDGET_RECORD_SHA256

IMPLEMENTATION_HEAD = "a" * 40
CODE_SNAPSHOT = "c" * 64


def release_document(protocol: R3FTransportProtocol) -> dict[str, object]:
    return {
        "schema_version": "ts-v5-r3f-llm-execution-release-v1",
        "release_id": "ts-v5-r3f-bound-proposal-canary-001",
        "status": "TS_V5_R3F_RESULT_BEFORE_EXECUTION_FROZEN",
        "execution_authorized": True,
        "production_authorization": "none",
        "user_approval": {
            "source": "user_explicit_approval_primary_codex_thread",
            "approved_on": "2026-08-13",
            "approved_scope_path": "config/ts_v5_r3f_llm_canary_scope_v1.yaml",
        ...
        "runtime": {
            ...
            "image_tag": "shaiwei:ts-v5-r3f-canary-001",
            "image_id": "sha256:" + "d" * 64,
            "code_snapshot_sha256": CODE_SNAPSHOT,
            "output_root": "data/research/trend_swing/ts-v5-r3f-canary-001",
            "attempt_ledger": "ledger/ts_v5_r3f_llm_attempts.csv",
            "transport_ledger": "ledger/ts_v5_r3f_llm_transports.csv",
        },
        "pre_execution_gates": {
            ...
            "single_transport_attempt_per_slot": True,
...
def paths(tmp_path: Path) -> tuple[Path, Path, Path, Path, Path]:
    ...
    output = project / "data/research/trend_swing/ts-v5-r3f-canary-001"
    attempt = project / "ledger/ts_v5_r3f_llm_attempts.csv"
    transport = project / "ledger/ts_v5_r3f_llm_transports.csv"
    ...
    release.write_text(
        yaml.safe_dump(release_document(R3FTransportProtocol.load()), sort_keys=False),
        encoding="utf-8",
    )
...
def completion(request: httpx.Request) -> httpx.Response:
    ...
    return httpx.Response(200, json={
        ...
        "choices": [{
            "message": {
                "content": json.dumps(minimal_bound_proposal(mechanism), ensure_ascii=False)
            },
            "finish_reason": "stop",
        }],
        "usage": {
...
def test_scope_preflight_and_gate_are_exact() -> None:
    report = preflight()
    assert report["gate"] == "GO_PREEXECUTION_ONLY"
    assert report["request_count"] == 6
    assert report["request_bundle_sha256"] == REQUEST_BUNDLE_SHA256
    assert report["provider_calls"] == 0
    assert batch_gate(6, 6) == "GO_BOUND_PROPOSAL_CANARY_ONLY"
    assert batch_gate(5, 5) == "STOP_INCOMPLETE_BATCH"


def test_release_and_provider_fail_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert R3FExecutionRelease.load(release, protocol).completed_responses_exact == 6
    document["authorization"]["transport_retry_authorized"] = True
    release.write_text(yaml.safe_dump(document, sort_keys=False), encoding="utf-8")
    with pytest.raises(D1ControlError, match="authorization"):
        R3FExecutionRelease.load(release, protocol)
    monkeypatch.setenv("DEEPSEEK_API_KEY", "must-not-be-used")
    with pytest.raises(D1ControlError, match="not authorized"):
        create_r3f_provider(
            protocol, release=None, attempt_id="blocked",
            transport_ledger_path=tmp_path / "transport.csv", artifact_root=tmp_path,
        )


def test_preflight_requires_pristine_bound_runtime(tmp_path: Path) -> None:
    ...
    assert report["maximum_transport_attempts_per_slot"] == 1
    assert report["secret_read"] is False


def test_six_mock_responses_reuse_and_offline_audit(tmp_path: Path) -> None:
    ...
    def provider_factory(protocol, **kwargs):
        ...
        return DeepSeekProvider(
            ...
            sleeper=lambda _: None, operator="fixture-ts-v5-r3f",
        )
    ...
    report, replay = run_batch(**kwargs), run_batch(**kwargs)
    audit = audit_batch(
        release_path=release, output_root=output, attempt_path=attempt,
        transport_path=transport, project_root=project,
    )
    assert calls == 6
    assert report["completed_response_count"] == report["external_request_count"] == 6
    assert report["valid_unique_candidate_count"] == 6
    assert report["transport_retries"] == 0
    assert report["gate"] == "GO_BOUND_PROPOSAL_CANARY_ONLY"
...
def test_retryable_error_makes_one_request_and_cannot_resume(tmp_path: Path) -> None:
    project, output, attempt, transport, release = paths(tmp_path)
    calls = 0
    ...
    def provider_factory(protocol, **kwargs):
        def handler(request: httpx.Request) -> httpx.Response:
            nonlocal calls
            calls += 1
            return httpx.Response(429, json={"error": "fixture"}, request=request)

        return DeepSeekProvider(
            protocol, attempt_id=kwargs["attempt_id"], api_key="fixture-key",
            transport_ledger_path=kwargs["transport_ledger_path"],
            artifact_root=kwargs["artifact_root"], transport=httpx.MockTransport(handler),
            execution_release=kwargs["release"], sleeper=lambda _: None,
        )
    ...
    with pytest.raises(D1ControlError):
        run_batch(**kwargs)
    assert calls == 1
    with pytest.raises(D1ControlError, match="cannot be resumed"):
        run_batch(**kwargs)
    assert calls == 1


def test_r3f_modules_and_compose_obey_isolation_limits() -> None:
    root = Path(__file__).resolve().parents[1]
    names = (
        "v5_r3f_canary.py", "v5_r3f_audit.py", "v5_r3f_release.py",
        "v5_r3f_live.py", "v5_r3f_result_audit.py", "v5_runtime_audit.py",
    )
    package = root / "src/shaiwei/research/trend_swing"
    assert all(len((package / name).read_text().splitlines()) <= 300 for name in names)
    compose = yaml.safe_load((root / "compose.ts-v5-r3f.yaml").read_text(encoding="utf-8"))
    live = compose["services"]["ts-v5-r3f"]
    ...
    for service in ("ts-v5-r3f-preflight", "ts-v5-r3f-audit", "ts-v5-r3f-replay"):
        assert compose["services"][service]["network_mode"] == "none"
        assert "DEEPSEEK_API_KEY" not in compose["services"][service]["environment"]
    for service in ("ts-v5-r3f-preflight", "ts-v5-r3f-replay"):
        assert all(item["read_only"] is True for item in compose["services"][service]["volumes"])
    assert all(
        item.get("read_only") is True
        for item in compose["services"]["ts-v5-r3f-audit"]["volumes"]
        if "/ledger/" in item["target"] or item["target"].startswith("/opt/")
    )
